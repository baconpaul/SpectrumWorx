...
std::array<::VstSpeakerArrangementType const, 8> const speakerArrangementTypes = {
    {kSpeakerArrMono, kSpeakerArrStereo, kSpeakerArr30Music, kSpeakerArr40Music, kSpeakerArr50,
     kSpeakerArr51, kSpeakerArr70Music, kSpeakerArr71Music}};
LE_MSVC_SPECIFIC(LE_WEAK_SYMBOL_CONST)
std::array<::VstSpeakerType const, 8> const speakerTypes = {{kSpeakerL, kSpeakerR, kSpeakerC,
                                                             kSpeakerLfe, kSpeakerLs, kSpeakerRs,
                                                             kSpeakerUndefined, kSpeakerUndefined}};

void fillMultiChannelSpeakerArrangement(::VstSpeakerArrangement &speakers,
                                        std::uint8_t const numberOfChannels)
{
    speakers.numChannels = numberOfChannels;
    speakers.type = (numberOfChannels < speakerArrangementTypes.size())
                        ? speakerArrangementTypes[numberOfChannels - 1]
                        : kSpeakerArrUserDefined;

    std::uint8_t speaker(0);
    std::uint8_t const definedSpeakerTypes(
        std::min(numberOfChannels, static_cast<std::uint8_t>(speakerTypes.size())));
...
void fillMultiChannelSpeakerArrangements(::VstSpeakerArrangement &input,
                                         ::VstSpeakerArrangement &output,
                                         std::uint8_t const numberOfMainChannels,
                                         std::uint8_t const numberOfSideChannels)
{
    if ((numberOfMainChannels == 1) && (numberOfSideChannels == 0))
    { // mono
    ...
    else if ((numberOfMainChannels == 1) && (numberOfSideChannels == 1))
...
void fillMultiMonoSpeakerArrangement(::VstSpeakerArrangement &speakers,
                                     std::uint8_t const numberOfChannels)
{
    speakers.numChannels = numberOfChannels;
...
void fillMultiMonoChannelSpeakerArrangements(::VstSpeakerArrangement &input,
                                             ::VstSpeakerArrangement &output,
                                             std::uint8_t const numberOfMainChannels,
                                             std::uint8_t const numberOfSideChannels)
{
...
bool SpectrumWorxVST24::getSpeakerArrangement(::VstSpeakerArrangement &input,
                                              ::VstSpeakerArrangement &output) const
{
        ...
        ((host().getNumOutputs() == numberOfMainChannels) ||
         (host().getNumOutputs() == maxNumberOfOutputs)) &&
            ((host().getNumInputs() == unsigned(numberOfMainChannels + numberOfSideChannels)) ||
             (host().getNumInputs() == maxNumberOfInputs)),
        "Performing a latency change notification in the middle of an IO mode change (which cannot "
        "be separated in VST 2.4)");
    ...
    fillMultiMonoChannelSpeakerArrangements(input, output, numberOfMainChannels,
                                            numberOfSideChannels);
    return true;
}

bool SpectrumWorxVST24::setSpeakerArrangement(::VstSpeakerArrangement const &input,
...
namespace
{
void fillChannelProperties(::VstPinProperties &properties, char const *const labelFormatString,
                           std::uint8_t const channelIndex, std::uint8_t const numberOfMainChannels)
{
    ...
    properties.arrangementType = (numberOfMainChannels < speakerArrangementTypes.size())
                                     ? speakerArrangementTypes[numberOfMainChannels - 1]
                                     : kSpeakerArrUserDefined;
}
} // anonymous namespace
...
void SpectrumWorxVST24::getInputProperties(std::uint8_t const index, ::VstPinProperties &properties)
{
    ...
    if (index < numberOfMainChannels)
        ...
        fillChannelProperties(properties, "SW side %1d",
                              static_cast<std::uint8_t>(index) - numberOfMainChannels,
                              numberOfMainChannels);
}

void SpectrumWorxVST24::getOutputProperties(std::uint8_t const index,
