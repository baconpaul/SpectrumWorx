...
GUID const CLSID_CResamplerMediaObject = {0xF447B69E, 0x1884, 0x4A7E, 0x80, 0x55, 0x34,
                                          0x6F,       0x74,   0xD6,   0xED, 0xB3};
#endif // LE_USE_RESAMPLER_DMO
...
class ConnectedOutputPin
{
  public:
    ...
    IPin *&operator=(IPin *const other)
    {
        pConnectedOutputPin_ = other;
        return pConnectedOutputPin_;
    }
...
class Sample::Impl : private IPin,
                     ...
                     private IEnumPins
{
  ...
  private:
    ...
    unsigned int remainingTypes() const
    {
        return static_cast<unsigned int>(types_.size() - currentType_);
    }
  ...
  private: // IEnumMediaTypes
    ...
    HRESULT STDMETHODCALLTYPE IEnumMediaTypes::Reset() override
    {
        currentType_ = 0;
        return S_OK;
    }
  ...
  private: // IMemInputPin
    ...
    HRESULT STDMETHODCALLTYPE NotifyAllocator(IMemAllocator * /*pAllocator*/,
    ...
    HRESULT STDMETHODCALLTYPE ReceiveMultiple(IMediaSample **, long nSamples,
  ...
  private: // IEnumPins
    ...
    HRESULT STDMETHODCALLTYPE IEnumPins::Reset() override
    {
...
Sample::Impl::Impl(std::uint32_t const allowedSamplerate)
    : state_(State_Stopped), pMyGraph_(0), currentType_(0), onlyPinAlreadyEnumerated_(false)
#ifdef _DEBUG
      ,
      hLog_(openLogFile())
#endif // _DEBUG
...
void Sample::Impl::fillType(DataType &type, unsigned int const sampleRate,
                            unsigned short const resolution)
{
    ...
    desiredFormat.nAvgBytesPerSec =
        desiredFormat.nSamplesPerSec * desiredFormat.wBitsPerSample / 8 * desiredFormat.nChannels;
...
HRESULT STDMETHODCALLTYPE Sample::Impl::QueryInterface(IID const &iid, void **const ppvObject)
{
    ...
    else if (iid == __uuidof(IEnumMediaTypes))
    {
        *ppvObject = static_cast<IEnumMediaTypes *>(this);
        return S_OK;
    }
    else if (iid == __uuidof(IEnumPins))
    {
...
HRESULT STDMETHODCALLTYPE Sample::Impl::Next(ULONG const cMediaTypes,
                                             ...
                                             ULONG *const pcFetched)
{
    ...
    *ppMediaTypes = 0;
    ...
    {
        AM_MEDIA_TYPE *const pMediaType(
        ...
        *pMediaType = types_[currentType_].type;
        ...
        std::memcpy(pMediaType->pbFormat, &types_[currentType_].type.pbFormat,
                    pMediaType->cbFormat);
...
HRESULT STDMETHODCALLTYPE Sample::Impl::Connect(IPin * /*pReceivePin*/,
                                                          ...
                                                          AM_MEDIA_TYPE const *const pmt)
{
    ...
    {
        if ((std::memcmp(&type.type, pmt, typeRelevantSize) == 0) &&
    ...
    case 3:
    {
        class Two24bitSamples
        {
          public:
            ...
            int firstSample32bit() const
            {
                return static_cast<int>((firstSample24bit() + 1) * 0xFF - 1);
            }
            int secondSample32bit() const
            {
                return static_cast<int>(secondSample24bit() << 8 | 0xFF);
            }

#ifdef _M_X64
            int secondSample24bit() const { return secondSample_; }
#else // avoid importing MSVC long long asm routines on 32 bit machines.
            int secondSample24bit() const
            {
                return (secondSampleFirstByte_ & 0xFF) | (secondSampleLastTwoBytes_ << 8);
            }
#endif
                                                 ...
                                                 Sample::DataHolder &data)
{
    ...
    //                                        (02.07.2010.) (Domagoj Saric)
    {
        ...
        if (((hr = pSourceFilter_.createInstance(CLSID_WMAsfReader)) != S_OK) ||
...
{
    {
        ...
        HRESULT const result(::CoInitializeEx(0, COINIT_MULTITHREADED | COINIT_DISABLE_OLE1DDE |
