...
///                                           (26.03.2014.) (Domagoj Saric)
{
  public:
    ...
    static ErrorCode makeErrorCode(bool const success)
    {
  ...
  public: // Plugin framework interface
    ErrorCode setParameter(ParameterID, Plugins::AutomatedParameterValue);
  ...
  public:
    bool isHost(Protocol, char const *const hostName) const
    {
        ...
        typename PluginPlatform::ProductStringBuf buffer;
        buffer[0] = 0;
        impl().host().getHostProductString(buffer);
        return std::strstr(buffer, hostName) != nullptr;
    }

    template <typename OtherProtocol> static bool isHost(OtherProtocol, char const * /*hostName*/)
    {
        return false;
    }
