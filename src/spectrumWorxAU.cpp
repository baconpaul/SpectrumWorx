...
std::uint8_t const
    startFrequencyIndex(IndexOf<BaseParameters::Parameters, BaseParameters::StartFrequency>::value);
std::uint8_t const
    stopFrequencyIndex(IndexOf<BaseParameters::Parameters, BaseParameters::StopFrequency>::value);
    ...
    SpectrumWorxCore const *LE_RESTRICT const pEffect)
{
    ...
    case ParameterID::ModuleChainParameter:
    {
        ...
        ParameterID moduleParameterID;
        moduleParameterID.value.type = ParameterID::ModuleParameter;
        ...
        for (std::uint8_t moduleParameterIndex(0); moduleParameterIndex < numberOfModuleParameters;
             ++moduleParameterIndex)
        {
            ...
            {
                for (std::uint8_t lfoParameterIndex(0);
                     lfoParameterIndex < ParameterCounts::lfoExportedParameters;
                     ++lfoParameterIndex, ++pDependentParameter)
                {
    ...
    case ParameterID::ModuleParameter:
    {
        ...
        {
        case startFrequencyIndex:
            moduleParameterID.value._.module.moduleParameterIndex = stopFrequencyIndex;
            break;
        case stopFrequencyIndex:
            moduleParameterID.value._.module.moduleParameterIndex = startFrequencyIndex;
            break;
    ...
    case ParameterID::LFOParameter:
    {
        ...
        case lowerBoundIndex:
            lfoParameterID.value._.lfo.lfoParameterIndex = upperBoundIndex;
            break;
        case upperBoundIndex:
            lfoParameterID.value._.lfo.lfoParameterIndex = lowerBoundIndex;
            break;
            LE_DEFAULT_CASE_UNREACHABLE();
        }
        BOOST_ASSERT(!pEffect ||
                                            ...
                                            SpectrumWorxCore const *LE_RESTRICT const pEffect)
{
    ...
    case ParameterID::ModuleChainParameter:
        ...
        {
            auto const pModule(
                pEffect->moduleChain().module(parameterID.value._.moduleChain.moduleIndex));
            if (pModule)
                return pModule->numberOfParameters() + pModule->numberOfLFOControledParameters() *
        ...
        {
            return Constants::maxNumberOfParametersPerModule +
                   (Constants::maxNumberOfParametersPerModule - 1) *
                       ParameterCounts::lfoExportedParameters;
        }
