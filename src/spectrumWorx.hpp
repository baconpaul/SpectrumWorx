...
Target convertParameterValueToLinearValue(Parameter const &source);
} // namespace Parameters
//------------------------------------------------------------------------------
namespace SW
{
...
class LE_NOVTABLE SpectrumWorx : public SpectrumWorxCore, public Plugin2HostInteropControler
{
  public:
  ...
  public:
    ...
    bool enableSideChannelInput(bool const enable)
    {
        ...
        return setNumberOfChannelsFromHost(enable ? numberOfChannels * 2 : numberOfChannels,
                                           numberOfChannels);
    }

    bool setNumberOfChannels(std::uint8_t const numberOfInputChannels,
                             std::uint8_t const numberOfOutputChannels)
    {
        return setNumberOfChannelsFromHost(numberOfInputChannels, numberOfOutputChannels);
    }
  ...
  public:
    ...
    unsigned int saveProgramState(std::uint8_t programIndex, void *pData,
  ...
  protected:
    ...
    void resetForGlobalParameters(Parameters const &);
    bool canParameterBeAutomated(ParameterID, Program const *) const;
  ...
  protected:                       // Global parameter change wrapper functions
    ...
    bool setGlobalParameter(FFTSize &, FFTSize ::param_type);
    bool setGlobalParameter(OverlapFactor &, OverlapFactor ::param_type);
#if LE_SW_ENGINE_INPUT_MODE >= 2
    bool setGlobalParameter(InputMode &, InputMode ::param_type);
#endif // LE_SW_ENGINE_INPUT_MODE >= 2
#if LE_SW_ENGINE_WINDOW_PRESUM
    bool setGlobalParameter(WindowSizeFactor &, WindowSizeFactor::param_type);
#endif // LE_SW_ENGINE_WINDOW_PRESUM
    bool setGlobalParameter(GlobalParameters::InputGain &parameter, float const newValue)
    {
        parameter.setValue(newValue);
        updateGUIForGlobalParameterChange();
        return true;
    }
    bool setGlobalParameter(GlobalParameters::OutputGain &parameter, float const newValue)
    {
        parameter.setValue(newValue);
        updateGUIForGlobalParameterChange();
        return true;
    }
    bool setGlobalParameter(GlobalParameters::MixPercentage &parameter, float const newValue)
    {
        parameter.setValue(newValue);
        updateGUIForGlobalParameterChange();
        return true;
  ...
  public:
    bool shouldLoadLastSessionOnStartup() const
    {
        return !runningAsAU() && loadLastSessionOnStartup_;
    }
    void shouldLoadLastSessionOnStartup(bool const loadLastSessionOnStartup)
    {
        loadLastSessionOnStartup_ = loadLastSessionOnStartup;
    }
...
inline void intrusive_ptr_add_ref(SpectrumWorx const *const pEffect)
{
    LE_ASSUME(pEffect); /*...mrmlj...for generic GUI::postMessage...*/
}
inline void intrusive_ptr_release(SpectrumWorx const *const pEffect)
{
    LE_ASSUME(pEffect); /*...mrmlj...for generic GUI::postMessage...*/
}
