...
Theme::~Theme() { registerFonts(false); }
...
bool Theme::shouldUpdateLFOControl(ModuleControlBase const &control)
{
    ...
    return (lfoUpdateBehaviour == Always) ||
    ...
    return (lfoUpdateBehaviour == Always) ||
           ((pActiveControl && (&pActiveControl->moduleUI() == &moduleUI)) &&
            ((lfoUpdateBehaviour == WhenControlActive) ||
             (lfoUpdateBehaviour == WhenControlSelected &&
              Detail::hasDirectFocus(pActiveControl->widget()))));
}
...
void Theme::drawLinearSliderBackground(juce::Graphics &g, int const x, int const y, int const width,
                                       int const height, float /*sliderPos*/,
                                       ...
                                       juce::Slider & /*slider*/)
{
    ...
    g.drawHorizontalLine(y + (static_cast<unsigned int>(height) / 2), Math::convert<float>(x),
                         Math::convert<float>(x + width));
}

namespace
{
void paintSliderThumb(juce::Graphics &graphics, juce::Image const &image, float const &position,
                      unsigned int const sliderVerticalPosition, unsigned int const sliderHeight,
                      bool const enlarge)
{
    ...
    unsigned int const horizontalPosition(Math::convert<unsigned int>(position) -
                                          (scaledThumbWidth / 2));
    unsigned int const verticalPosition(sliderVerticalPosition + (sliderHeight / 2) -
                                        (scaledThumbHeight / 2));
...
void Theme::drawLinearSliderThumb(juce::Graphics &g, int const /*x*/, int const y,
