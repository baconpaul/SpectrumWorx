...
void Detail::BandGainImpl::setup(IndexRange const &workingRange, Engine::Setup const &engineSetup)
{
    ...
    unsigned int const currentLeftAlignment(reinterpret_cast<std::size_t>(downSlope_.begin()) /
                                            sizeof(float) % alignment);
    unsigned int const currentRightAlignment(reinterpret_cast<std::size_t>(pUpSlope_) /
                                             sizeof(float) % alignment);
    if ((windowSize != currentWindowSize) || (leftAlignmentOffset != currentLeftAlignment) ||
        (rightAlignmentOffset != currentRightAlignment) || (attenuation_ != attenuation))
    {
        ...
        pUpSlope_ = window_.begin() +
                    ((slopeSize + leftAlignmentOffset + alignment - 1) & ~(alignment - 1)) +
...
void BandpassImpl::process(Engine::ChannelData_AmPh data, Engine::Setup const &) const
{
    ...
    unsigned int rightSize(
        std::min<unsigned int>(data.full().numberOfBins() - data.endBin(), slopeSize));
    ...
    while (leftSize--)
    {
        *pDataUpSlope++ *= *pWindowUpSlope++;
    }
    while (rightSize--)
    {
        *pDataDownSlope++ *= *pWindowDownSlope++;
    }
#else
