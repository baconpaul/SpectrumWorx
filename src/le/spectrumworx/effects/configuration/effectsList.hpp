                    ...
                    x(sumo_pitch, sumoPitch, SumoPitch, Pitch)                                     \
                        x(pitch_spring, pitchSpring, PitchSpring, Pitch)                           \
                            x(octaver, octaver, Octaver, Pitch)                                    \
                                                                                                   \
        /* Timbre */                                                                               \
        x(bandpass, bandpass, Bandpass, Timbre) x(bandpass, bandpass, Bandstop, Timbre)            \
            x(ah_ah, ahAh, AhAh, Timbre) x(smoother, smoother, Smoother, Timbre)                   \
                x(sharper, sharper, Sharper, Timbre)                                               \
                    x(centroid_extractor, centroidExtractor, CentroidExtractor, Timbre)            \
                        x(tonal, tonal, Tonal, Timbre) x(tonal, tonal, Atonal, Timbre)             \
                                                                                                   \
        /* Time */                                                                                 \
        x(freeze, freeze, Freeze, Time) x(slicer, slicer, Slicer, Time)                            \
            x(wobbler, wobbler, Wobbler, Time) x(reverser, reverser, Reverser, Time)               \
                x(eximploder, exImploder, Imploder, Time)                                          \
                    x(eximploder, exImploder, Exploder, Time)                                      \
                                                                                                   \
        /* Space */                                                                                \
        x(frecho, frecho, Frecho, Space) x(frecho, frecho, Frevcho, Space)                         \
            x(freqverb, freqverb, Freqverb, Space)                                                 \
                                                                                                   \
        /* Phase */                                                                                \
        x(robotizer, robotizer, Robotizer, Phase) x(whisperer, whisperer, Whisperer, Phase)        \
            x(phasevolution, phasevolution, Phasevolution, Phase) x(phlip, phlip, Phlip, Phase)    \
                                                                                                   \
        /* Loudness */                                                                             \
        x(gain, gain, Gain, Loudness) x(exaggerator, exaggerator, Exaggerator, Loudness)           \
            x(denoiser, denoiser, Denoiser, Loudness)                                              \
                x(quiet_boost, quietBoost, QuietBoost, Loudness)                                   \
                    x(freqnamics, freqnamics, Freqnamics, Loudness)                                \
                                                                                                   \
        /* Combine */                                                                              \
        x(talking_wind, talkingWind, TalkingWind, Combine) x(                                      \
            convolver, convolver, Convolver, Combine) x(ethereal, ethereal, Ethereal, Combine)     \
            x(vaxateer, vaxateer, Vaxateer, Combine) x(shapeless, shapeless, Shapeless, Combine)   \
                x(colorifer, colorifer, Colorifer, Combine) x(merger, merger, Merger, Combine)     \
                    x(blender, blender, Blender, Combine) x(inserter, inserter, Inserter, Combine) \
                            ...
                            x(pitch_spring, pitchSpring, PitchSpringPVD, PVD)                      \
                                x(eximploder, exImploder, PVImploder, PVD)                         \
                                    x(eximploder, exImploder, PVExploder, PVD)                     \
                                        x(phase_vocoder_synthesis, phaseVocoderSynthesis,          \
                                          PhaseVocoderSynthesis, PVD)                              \
                                                                                                   \
        /* Misc */                                                                                 \
        x(armonizer, armonizer, Armonizer, Misc) x(slew_limiter, slewLimiter, SlewLimiter, Misc)   \
            x(shifter, shifter, Shifter, Misc) x(swappah, swappah, Swappah, Misc)                  \
