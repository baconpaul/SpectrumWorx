...
{
    {
        ...
        std::memcpy(shiftedAmPh1.amplitudes.begin(), data.input_.amplitudes.begin(),
                    num_bins_ * sizeof(float));
        std::memcpy(shiftedAmPh1.phases.begin(), data.input_.phases.begin(),
                    num_bins_ * sizeof(float));

        std::memcpy(shiftedAmPh2.amplitudes.begin(), data.input_.amplitudes.begin(),
                    num_bins_ * sizeof(float));
        std::memcpy(shiftedAmPh2.phases.begin(), data.input_.phases.begin(),
                    num_bins_ * sizeof(float));
        ...
        Math::amph2ReIm(shiftedAmPh1.amplitudes.begin(), shiftedAmPh1.phases.begin(),
                        shifted1Reals_.begin(), shifted1Imags_.begin(), num_bins_);

        Math::amph2ReIm(shiftedAmPh2.amplitudes.begin(), shiftedAmPh2.phases.begin(),
                        shifted2Reals_.begin(), shifted2Imags_.begin(), num_bins_);
    }
