...
void FrechoImpl::setup(IndexRange const &, Engine::Setup const &engineSetup)
{
    ...
    // distance:
    echoSizeInSteps_ = static_cast<std::uint8_t>(
        engineSetup.milliSecondsToSteps(parameters().get<Distance>() * 2 * 1000 / speedOfSound));
...
void FrechoImpl::doProcess(ChannelState &cs, Engine::ChannelData_ReIm &target,
                           Engine::Setup const &engineSetup) const
{
    ...
    float *LE_RESTRICT const pEchoFromFrameReals(
        &cs.historyBuffer[cs.frameCounter * alignedNumberOfBins * 2]);
    ...
    if (!ps_.skipProcessing())
    {
        ...
        Math::reim2AmPh(pEchoFromFrameReals, pEchoFromFrameImags,
                        pitchShiftedEcho.full().amps().begin(),
                        pitchShiftedEcho.full().phases().begin(), numberOfBins);

        ps_.process(cs.pvState, std::forward<Engine::ChannelData_AmPh>(pitchShiftedEcho));

        Math::amph2ReIm(pitchShiftedEcho.full().amps().begin(),
                        pitchShiftedEcho.full().phases().begin(), pEchoFromFrameReals,
                        pEchoFromFrameImags, numberOfBins);
    }
}

void FrechoImpl::process(ChannelState &cs, Engine::ChannelData_ReIm data,
                         Engine::Setup const &engineSetup) const
{
...
void FrevchoImpl::process(ChannelState &cs, Engine::ChannelData_ReIm data,
                          Engine::Setup const &engineSetup) const
{
    ...
    swap(data.reals().begin(), data.reals().end(),
         historyData.targetHistory.pAmplitudesOrReals + startBin);
    swap(data.imags().begin(), data.imags().end(),
    ...
    if (historyData.isEmulated())
    {
        ...
        copy(historyData.sourceHistory.pAmplitudesOrReals + startBin, data.reals().begin(),
             numberOfBins);
        copy(historyData.sourceHistory.pPhasesOrImags + startBin, data.imags().begin(),
             numberOfBins);
    }
