...
class SynthImpl : public EffectImpl<Synth>
{
  ...
  public: // LE::Effect required interface.
    SynthImpl()
        : lastFreq_(0), lastWindow_(static_cast<Engine::Constants::Window>(-1)), lastFFTSize_(0)
    {
    }
    ...
    struct ChannelState
    {
        ...
        void reset();
        void resize(Engine::StorageFactors const &, Engine::Storage &);
        static std::uint32_t requiredStorage(Engine::StorageFactors const &);
    }; // struct ChannelState
    ...
    void process(ChannelState &, Engine::MainSideChannelData_AmPh, Engine::Setup const &)
        const; //...mrmlj..._ABI due to msvc12 bogus compilation errors...
