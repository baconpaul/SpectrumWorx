...
void VibratoEffect::setup(unsigned int const &periodInMilliseconds,
                          Engine::Setup const &engineSetup)
{
    period_ = engineSetup.milliSecondsToSteps(periodInMilliseconds);
}

float VibratoEffect::calculateNewPitch(ChannelState &cs,
                                       CommonParameters::SpringType const springType,
                                       unsigned int const vibratoDepthInSemitones) const
{
    ...
    {
    case CommonParameters::SpringType::Symmetric:
        offsetAmplitude = currentAmplitude;
        break;
    case CommonParameters::SpringType::Up:
        offsetAmplitude = (1 + currentAmplitude) / 2;
        break;
    case CommonParameters::SpringType::Down:
        offsetAmplitude = -(1 + currentAmplitude) / 2;
        break;
