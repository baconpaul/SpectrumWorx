                                  ...
                                  Engine::Setup const &engineSetup) const
{
...

    {
        ...
        float const hfExtraAttenuationPerBin(parameters().get<HFAbsorption>() /
                                             float(engineSetup.numberOfBins()));
        ...
        while (pSignalReal != pEnd)
        {
            ...
            float const attenuationThisBin(
                Math::dB2NormalisedLinear(-60 * secondsPerStep / time60dBThisBin));
            ...
            bool const binWithinWorkingRange((&signalReal >= data.reals().begin()) &&
...

    {
        ...
        amph2ReIm(data2.full().amps().begin(), data2.full().phases().begin(),
                  cs.feedbackSumReals.begin(), cs.feedbackSumImags.begin(), noEchoBin_);
    }
}
