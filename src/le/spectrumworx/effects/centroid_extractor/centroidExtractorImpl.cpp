...
void CentroidExtractorImpl::process(ChannelState &cs, Engine::ChannelData_AmPh data,
                                    Engine::Setup const &setup) const
{
    ...
    {
    case Mode::Centroid:
        desiredCentralBin = centroid(data.full().amps());
        break;
    case Mode::Peak:
        desiredCentralBin = maxPeak(data.full().amps(), setup);
        break;
    case Mode::Dominant:
        desiredCentralBin = dominant(data.full().amps(), setup, cs);
        break;
    ...
    float const slopeOffset(std::max(0.0f, convert<float>(lBin - desiredLeftBin)) *
                            slopeIncrement); // Clang calls the vector version if just 0 is used...
...
IndexRange::value_type CentroidExtractorImpl::dominant(ReadOnlyDataRange const &amplitudes,
                                                       Engine::Setup const &engineSetup,
                                                       ChannelState &cs) const
{
    using namespace Math;
    float const c(
        PitchDetector::findPitch(amplitudes, cs, 50, 50 * 2 * 2 * 2 * 2 * 2, engineSetup));
...
IndexRange::value_type CentroidExtractorImpl::maxPeak(ReadOnlyDataRange const &amplitudes,
