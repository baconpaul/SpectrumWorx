...
namespace
{
bool domainDirectory(::KNOWNFOLDERID const &knownFolder, Path &path)
{
    wchar_t *pPath;
    HRESULT const result(::SHGetKnownFolderPath(
        knownFolder, KF_FLAG_DONT_VERIFY | KF_FLAG_DONT_UNEXPAND | KF_FLAG_NO_ALIAS, nullptr,
        &pPath));
    if (FAILED(result))
    {
        LE_ASSERT(!pPath);
...
template <> bool pathFor<Documents>(Path &buffer)
{
    return domainDirectory(FOLDERID_Documents, buffer);
}
template <> bool pathFor<SharedDocuments>(Path &buffer)
{
    return domainDirectory(FOLDERID_PublicDocuments, buffer);
}
template <> bool pathFor<Library>(Path &buffer)
{
    return domainDirectory(FOLDERID_Libraries, buffer);
}
...
template <> bool pathFor<Temporaries>(Path &buffer)
{
