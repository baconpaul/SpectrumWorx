...
template <typename T> struct LERealGeneratePolicies : boost::spirit::karma::real_policies<T>
{
    using Base = boost::spirit::karma::real_policies<T>;

    LERealGeneratePolicies(std::uint8_t const precision = sizeof(T) == sizeof(float) ? 4 : 9)
    ...
    static bool dot(OutputIterator &sink, T const n, std::uint8_t const precision)
    {
        if (precision && n)
    ...
    template <typename OutputIterator>
    bool fraction_part(OutputIterator &sink, T const n, std::uint8_t const actualPrecision,
                       std::uint8_t const precision) const
    {
...
template <typename T>
LE_NOTHROWNOALIAS unsigned int generateFloat(T const value, std::uint8_t const precision,
                                             char *const pBuffer)
{
    char *pOutput(pBuffer);
    BOOST_VERIFY(boost::spirit::karma::generate(
        pOutput, boost::spirit::karma::real_generator<T, LERealGeneratePolicies<T>>(precision),
        value));
    *pOutput = '\0';
...
{
#ifdef LE_USE_SPIRIT
    return lexical_cast(static_cast<float>(value), std::min<std::uint8_t>(decimalPlaces, 4),
                        buffer);
#else
    ...
    BOOST_ASSERT_MSG((std::abs(lexical_cast<float>(buffer) - static_cast<float>(value)) <
...
template <> LE_COLD int lexical_cast<int>(char const *valueString)
{
    ...
    int result;
    BOOST_VERIFY((qi::extract_int<int, 10, 1, maxDigits>::call(valueString, valueString + maxDigits,
                                                               result)));
    return result;
#else
...
template <typename T> struct LERealParsePolicies : boost::spirit::qi::real_policies<T>
{
    static bool const allow_leading_dot = false;

    template <typename Iterator> static bool parse_exp(Iterator &first, Iterator const &last)
    {
        BOOST_ASSERT(first == last || (*first != 'e' && *first != 'E'));
        return false;
    }

    template <typename Iterator> static bool parse_exp_n(Iterator &, Iterator const &, int &)
    {
        LE_UNREACHABLE_CODE();
    }
...
float lexical_cast_float_worker(char const *&pValueString)
{
    ...
    BOOST_VERIFY(Parser::parse(pValueString, pValueString + RequiredStringStorage<float>::value,
                               result, LERealParsePolicies<float>()));
    return result;
#else
