...
using EnvPtr =
    std::unique_ptr<JNIEnv, Detail::EnvDeleter>; ///< Thread-local JNI environment smart ptr
template <typename JavaType = jobject>
using GlobalRef = std::unique_ptr<
    typename std::remove_pointer<JavaType>::type,
    Detail::GlobalRefDeleter>; ///< Global JNI reference to a <VAR>JavaType</VAR> Java object
template <typename JavaType = jobject>
using LocalRef = std::unique_ptr<
    typename std::remove_pointer<JavaType>::type,
    Detail::LocalRefDeleter>; ///< Local JNI reference to a <VAR>JavaType</VAR> Java object
...
inline JavaVM &vm()
{
    LE_ASSUME(Detail::pJVM);
    return *Detail::pJVM;
}

/// \brief Set the singleton JavaVM instance
inline void setVM(::JavaVM &vm)
{
...
namespace Detail
{
inline jobject newGlobalReference(jobject object)
{
    return newGlobalReference(preAttachedEnv(), object);
}
inline jobject newLocalReference(jobject object)
{
    return newLocalReference(preAttachedEnv(), object);
}
} // namespace Detail

template <typename JavaType> GlobalRef<JavaType> globalReference(JavaType const javaObject)
{
    return {static_cast<JavaType>(Detail::newGlobalReference(javaObject)),
            Detail::GlobalRefDeleter()};
} ///< Create a global reference to a Java object.
template <typename JavaType> LocalRef<JavaType> localReference(JavaType const javaObject)
{
    return {static_cast<JavaType>(Detail::newLocalReference(javaObject)),
            Detail::LocalRefDeleter()};
} ///< Create a local reference to a Java object.

template <typename T> jlong marshalPointer(T *const cPtr)
{
    return reinterpret_cast<std::intptr_t>(cPtr);
} ///< Store a native pointer into an opaque integer data type for passing to Java code.
template <typename T> T *unmarshalPointer(jlong const jniPtr)
{
    return reinterpret_cast<T *>(static_cast<std::intptr_t>(jniPtr));
} ///< Extract a native pointer from the opaque integer data type in which it was stored on the Java side.
