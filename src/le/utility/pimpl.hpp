...
namespace std
{
template <std::size_t size> struct aligned_storage<size, 16>
{
    struct __declspec(align(16)) type
    {
        __declspec(align(16)) char storage[size];
    };
};
} // namespace std
#pragma warning(pop)
...
template <class Interface, class BaseInterface> class PImpl;

template <class Interface> struct Implementation;
...
template <class Interface, class BaseInterface> struct StackPImplBase
{
    typedef BaseInterface type;
};
template <class Interface> struct StackPImplBase<Interface, PImplTerminator>
{
    typedef PImpl<Interface, PImplTerminator> type;
};
} // namespace Detail
...
template <class Interface, std::size_t storageSize, std::size_t alignment = sizeof(void *),
          class BaseInterface = Detail::PImplTerminator>
