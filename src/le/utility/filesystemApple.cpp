...
#else // OS X
template <> NSString *LE_COLD pathFor<AppData>()
{
...
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
template <> NSString *LE_COLD pathFor<Temporaries>()
{
    return [pathFor<AppData>() stringByAppendingPathComponent:@"tmp"];
}
#else // OS X
...
Result LE_COLD PathResolver<rootDirectory>::apply(char const *const relativePath, Functor const f)
{
    @autoreleasepool
    {
        ...
        NSString *const absolutePath([pathFor<rootDirectory>()
            stringByAppendingPathComponent:ObjC::utf8String(relativePath)]);
...
Result LE_COLD PathResolver<Resources>::apply(char const *const relativePath, Functor const f)
{
    @autoreleasepool
    {
        ...
        return f(
            filePath
                ? [filePath UTF8String]
                : relativePath); //...mrmlj...revisit: why are we supporting (erroneusly passed?) absolute paths here?
    }
}
