...
//                                            (30.08.2011.) (Domagoj Saric)
      ...
      (defined(__ANDROID__) && defined(__GLIBCXX__)))
namespace std
{
...
template <class T> struct LE_IDENTITY_TPL_NAME
{
    typedef T type;
    T const &operator()(T const &left) const { return left; }
};

template <class T> inline T &&forward(typename LE_IDENTITY_TPL_NAME<T>::type &arg)
{
    return static_cast<T &&>(arg);
}

template <class T> inline typename remove_reference<T>::type &&move(T &&arg)
{
