...
template <> struct value_of_impl<LE::Parameters::IteratorTag>
{
    template <typename Iterator> struct apply;

    template <typename Parameters, unsigned int Index>
    struct apply<LE::Parameters::Iterator<Parameters, Index>>
        : Parameters::template ParameterAt<Index>
    {
    };
};

template <typename Tag> struct deref_impl;
template <> struct deref_impl<LE::Parameters::IteratorTag>
{
    template <typename Iterator> struct apply;
    ...
    struct apply<LE::Parameters::Iterator<Parameters, Index>>
    {
        ...
        typedef typename mpl::if_<std::is_const<Parameters>, value_type const &, value_type &>::type
            type;
...
template <> struct next_impl<LE::Parameters::IteratorTag>
{
    template <typename Iterator> struct apply;
    ...
    struct apply<LE::Parameters::Iterator<Parameters, Index>>
    {
        ...
        static type call(::LE::Parameters::Iterator<Parameters, Index> const it)
        {
            return type(it.parameters_);
        }
    };
};

template <typename Tag> struct prior_impl;
template <> struct prior_impl<LE::Parameters::IteratorTag>
{
    template <typename Iterator> struct apply;
...
template <> struct distance_impl<LE::Parameters::IteratorTag>
{
...
{
    template <typename Parameters> struct apply
    {
...
{
    template <typename Parameters, typename Key> struct apply
    {
        typedef Key type;
    };
};
...
{
    template <typename Parameters, typename N> struct apply
    {
