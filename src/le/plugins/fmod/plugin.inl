...
#ifndef NDEBUG
template <class Impl> unsigned int Plugin<Impl, Protocol::FMOD>::pluginInstanceCount_(0);
#endif
...
FMOD_RESULT F_CALLBACK Plugin<Impl, Protocol::FMOD>::create(FMOD_DSP_STATE *LE_RESTRICT const pDSP)
{
    ...
    if (pImpl.get() && Impl::makeErrorCode(pImpl->initialise()) == Success &&
                                                          ...
                                                          int *LE_RESTRICT const pOutChannels)
{
    ...
    {
        LE_TRACE("FMOD changed the channel configuration in the middle of processing (%u -> %u).",
                 impl.numberOfChannels(), numberOfMainChannels);
        if (!impl.setNumberOfChannels(numberOfMainChannels + numberOfSideChannels,
...
template <class Impl> FMOD_DSP_DESCRIPTION *Plugin<Impl, Protocol::FMOD>::getDescription()
{
    ...
    static FMOD_DSP_DESCRIPTION /*const*/ description = {
        ...
        "" /*Impl::name*/,
        (Impl::versionMajor * 0x10000) | (Impl::versionMinor * 0x1000) |
            (Impl::versionPatch * 0x100), // version 0xAAAABBBB   A = major, B = minor.
        1,
        1,
        &Plugin::create,
        &Plugin::release,
    ...
    if (!*description.name)
    {
                 ...
                 parameterDescriptions.begin(), parameterDescriptions.end() - useSideChain))
        {
            Impl::getParameterProperties(
                index, LE_MSVC_SPECIFIC(const_cast<ParameterInformation &>)(parameterInfo),
                nullptr);
#ifdef _DEBUG
            FMOD_DSP_PARAMETER_DESC const &info(parameterInfo.fmodStructure());
            std::printf(
                "Parameter %u, %s: type %d min %.2f, max %.2f, default %.2f, unit (%s).\n",
                index.value, info.name, info.type,
                (info.type == FMOD_DSP_PARAMETER_TYPE_FLOAT) ? info.floatdesc.min
                                                             : static_cast<float>(info.intdesc.min),
                (info.type == FMOD_DSP_PARAMETER_TYPE_FLOAT) ? info.floatdesc.max
                                                             : static_cast<float>(info.intdesc.max),
                (info.type == FMOD_DSP_PARAMETER_TYPE_FLOAT)
                    ? info.floatdesc.defaultval
                    : static_cast<float>(info.intdesc.defaultval),
                info.label);
#endif // _DEBUG
            index.value++;
        }
