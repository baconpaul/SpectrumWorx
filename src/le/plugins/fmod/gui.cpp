                                               ...
                                               float const parameterValue) const
{
    ...
    case FMOD_DSP_PARAMETER_TYPE_FLOAT:
        value.floatvalue = static_cast<float>(parameterValue);
        LE_ASSUME(parameterValue >= desc.floatdesc.min && parameterValue <= desc.floatdesc.max);
        break;
    case FMOD_DSP_PARAMETER_TYPE_INT:
        value.intvalue = static_cast<int>(parameterValue);
        LE_ASSUME(parameterValue >= desc.intdesc.min && parameterValue <= desc.intdesc.max);
        break;
    case FMOD_DSP_PARAMETER_TYPE_BOOL:
        value.boolvalue = parameterValue != 0;
        LE_ASSUME(parameterValue == 0 || parameterValue == 1);
        break;
...
void FMODEditorBase::syncStudioWindowSize(WindowHandle const studioWindowHandle,
                                          unsigned int const width, unsigned int const height)
{
    ...
    auto const newStudioViewSize(
        NSMakeSize(studioViewSize.width + width, studioViewSize.height + height));
