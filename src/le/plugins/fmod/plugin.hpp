...
class FMODHostProxy
{
  ...
  public: // Plugin information/state reported/visible to host.
    bool reportNewNumberOfIOChannels(std::uint_fast8_t inputs, std::uint_fast8_t sideInputs,
                                     std::uint_fast8_t outputs);
...
template <> struct ErrorCode<Protocol::FMOD>
{
    enum value_type
    {
        Success = FMOD_OK,
...
template <> struct AutomatedParameterFor<Protocol::FMOD>
{
...
class FMODPluginBase : private FMODHostProxy, public ErrorCode<Protocol::FMOD>
{
...
template <> class ParameterInformation<Protocol::FMOD> : private FMOD_DSP_PARAMETER_DESC
{
  public:
  ...
  private:
    template <class Parameter> void setValues(Parameters::BooleanParameterTag)
    {
        type = FMOD_DSP_PARAMETER_TYPE_BOOL;
    ...
    template <class Parameter> void setValues(Parameters::LinearIntegerParameterTag)
    {
        type = FMOD_DSP_PARAMETER_TYPE_INT;
    ...
    template <class Parameter> void setValues(Parameters::EnumeratedParameterTag)
    {
        ...
        intdesc.valuenames =
            const_cast<char **>(LE::Parameters::DiscreteValues<Parameter>::strings);
    }

    template <class Parameter> void setValues(LE::Parameters::PowerOfTwoParameterTag)
    {
    ...
    {
        setValues<Parameter>(LE::Parameters::LinearFloatParameterTag());
    }

    template <unsigned int N> void setLabel(char const (&unit)[N])
    {
...
template <class ImplParam> class Plugin<ImplParam, Protocol::FMOD> : public FMODPluginBase
{
  protected:
...
namespace Detail
{
void setMemoryCallbacks(FMOD_DSP_STATE_SYSTEMCALLBACKS const &);
}
