...
/// -------------
///
/// Copyright (c) 2014. Little Endian Ltd.
...
class FMODEditorBase
{
  ...
  protected:
    ...
    static FMOD_RESULT F_CALLBACK setHostCallbacks(Editor2HostCallbacks const *const pCallbacks)
    {
        plugin2HostCallbacks_ = *pCallbacks;
        return FMOD_OK;
    }
...
template <class Impl> class FMODEditor : public FMODEditorBase
{
  public:
  ...
  private:
    ...
    static OptionalEditor *optionalEditorFromBase(FMODEditorBase *const pEditorBase,
                                                  bool const assumeConstructed = false)
    {
  ...
  private:
    ...
    static FMOD_RESULT F_CALLBACK show(void *pEditor, int windowHandle, void const *pState,
                                       unsigned int stateSize);
    static FMOD_RESULT F_CALLBACK getLocalState(void *pEditor, void **ppState,
                                                unsigned int *pStateSize);
    static FMOD_RESULT F_CALLBACK updateParameterValue(void *pEditor, int index,
                                                       AutomatedParameterValue const *);
    static FMOD_RESULT F_CALLBACK updateSharedState(void *pEditor, void const *pSharedState,
                                                    unsigned int sharedStateSize);
    static FMOD_RESULT F_CALLBACK printParameterValue(
        void *pEditor, unsigned int parameterIndex, FMOD::PLUGIN_PARAMETER_VALUE const &,
        FMOD::StudioAudioPlugin::ParameterValueStringBuffer &);
...
#define LE_PLUGIN_FMOD_GUI_ENTRY_POINT(implClass)                                                  \
    extern "C" F_DECLSPEC F_DLLEXPORT FMOD::StudioAudioPlugin::HOST_TO_EDITOR_CALLBACKS *F_STDCALL \
    FMODStudioAudioPluginEditorCallbacks()                                                         \
    {                                                                                              \
        return const_cast<FMOD::StudioAudioPlugin::HOST_TO_EDITOR_CALLBACKS *>(                    \
            &LE::Plugins::FMODEditor<implClass>::callbacks());                                     \
    }
#else
#define LE_PLUGIN_FMOD_GUI_ENTRY_POINT(implClass)                                                  \
    extern "C" FMOD::StudioAudioPlugin::HOST_TO_EDITOR_CALLBACKS *F_STDCALL                        \
    FMOD_LittleEndian_SpectrumWorx_StudioAudioPluginEditorCallbacks()                              \
    {                                                                                              \
        return const_cast<FMOD::StudioAudioPlugin::HOST_TO_EDITOR_CALLBACKS *>(                    \
            &LE::Plugins::FMODEditor<implClass>::callbacks());                                     \
    }
                                                         ...
                                                         unsigned int const stateSize)
{
    ...
    FMODEditorBase::WindowHandle const parentWindow(
        reinterpret_cast<FMODEditorBase::WindowHandle>(parentWindowHandle));
    ...
    std::uint_fast16_t const numberOfParameters(
        /*Impl::maxNumberOfParameters*/ unsigned(pUgh->numparameters) - 1 /*side chain parameter*/);
    for (Plugins::ParameterIndex parameterIndex(0); parameterIndex.value < numberOfParameters;
    ...
    void *const pEditor, int const index, AutomatedParameterValue const *LE_RESTRICT const pValue)
{
    ...
    case FMOD_DSP_PARAMETER_TYPE_FLOAT:
        value = pValue->floatvalue;
        break;
    case FMOD_DSP_PARAMETER_TYPE_INT:
        value = pValue->intvalue;
        break;
    case FMOD_DSP_PARAMETER_TYPE_BOOL:
        value = pValue->boolvalue;
        break;
                                      ...
                                      FMOD::StudioAudioPlugin::ParameterValueStringBuffer &buffer)
{
    ...
    {
    case FMOD_DSP_PARAMETER_TYPE_FLOAT:
        internalValue = value.floatvalue;
        break;
    case FMOD_DSP_PARAMETER_TYPE_INT:
        internalValue = value.intvalue;
        break;
    case FMOD_DSP_PARAMETER_TYPE_BOOL:
        internalValue = value.boolvalue;
        break;
        LE_DEFAULT_CASE_UNREACHABLE();
    }
