...
*/
  ...
  public:
    ...
    {
        return pTimeInfo_->samplePos;
    } ///< current Position in audio samples (always valid)
    double const &sampleRate() const
    {
        return pTimeInfo_->sampleRate;
    } ///< current Sample Rate in Herz (always valid)
    double const &systemTime() const
    {
        return pTimeInfo_->nanoSeconds;
    } ///< System Time in nanoseconds (10^-9 second)
    double const &numberOfBeats() const
    {
        return pTimeInfo_->ppqPos;
    } ///< Musical Position, in Quarter Note (1.0 equals 1 Quarter Note)
    double const &bpm() const
    {
        return pTimeInfo_->tempo;
    } ///< current Tempo in BPM (Beats Per Minute)
    double const &lastBarStartPosition() const
    {
        return pTimeInfo_->barStartPos;
    } ///< last Bar Start Position, in Quarter Note
    double const &cycleStartPosition() const
    {
        return pTimeInfo_->cycleStartPos;
    } ///< Cycle Start (left locator), in Quarter Note
    double const &cycleEndPosition() const
    {
        return pTimeInfo_->cycleEndPos;
    } ///< Cycle End (right locator), in Quarter Note
    unsigned int timeSignatureNumerator() const
    {
        return pTimeInfo_->timeSigNumerator;
    } ///< Time Signature Numerator (e.g. 3 for 3/4)
    unsigned int timeSignatureDenominator() const
    {
        return pTimeInfo_->timeSigDenominator;
    } ///< Time Signature Denominator (e.g. 4 for 3/4)
    unsigned int smpteOffset() const
    {
        return pTimeInfo_->smpteOffset;
    ...
    unsigned int smpteFrameRate() const
    {
        return pTimeInfo_->smpteFrameRate;
    } ///< @see VstSmpteFrameRate
    int samplesToNextClock() const
    {
        return pTimeInfo_->samplesToNextClock;
    } ///< MIDI Clock Resolution (24 Per Quarter Note), can be negative (nearest clock)

    template <FieldFlags desiredField> bool hasField() const
    {
        return (desiredField & alwaysTrueFlags) ||
               (pTimeInfo_ && (pTimeInfo_->flags & desiredField));
    }
    template <unsigned int desiredFields> bool hasFields() const
    {
        return pTimeInfo_ &&
               ((pTimeInfo_->flags & desiredFields) == (desiredFields & ~alwaysTrueFlags));
    }
...
class VSTHost24Proxy : private Detail::AEffectWrapper
{
  public:
    typedef
#if defined(_WIN32)
  ...
  public: // Host information/state.
    ::VstAutomationStates getAutomationState() const
    {
        return static_cast<::VstAutomationStates>(call(audioMasterGetAutomationState));
    }
    ::VstProcessLevels getCurrentProcessLevel() const
    {
        return static_cast<::VstProcessLevels>(call(audioMasterGetCurrentProcessLevel));
    }
    ::VstHostLanguage getHostLanguage() const
    {
        return static_cast<::VstHostLanguage>(call(audioMasterGetLanguage));
    }
    unsigned int getSupportedVSTVersion() const
    {
        return static_cast<unsigned int>(call(audioMasterVersion));
    }

    template <unsigned int filterMask> Detail::VST24TimingInformation getTimeInfo() const
    {
        return makeTimeInfo(call(audioMasterGetTime, 0, filterMask));
    }

    unsigned int getInputLatency() const
    {
        return static_cast<unsigned int>(call(audioMasterGetInputLatency));
    }
    unsigned int getOutputLatency() const
    {
        return static_cast<unsigned int>(call(audioMasterGetOutputLatency));
    }

    unsigned int getBlockSize() const
    {
        return static_cast<unsigned int>(call(audioMasterGetBlockSize));
    } ///< VSTSDK AudioEffectX::updateBlockSize () equivalent.
    unsigned int getSampleRate() const
    {
        return static_cast<unsigned int>(call(audioMasterGetSampleRate));
    } ///< VSTSDK AudioEffectX::updateSampleRate() equivalent.

  public: // Plugin information/state reported/visible to host.
    bool reportNewNumberOfIOChannels(unsigned int inputs, unsigned int sideInputs,
  ...
  public: // GUI.
    ...
    bool sizeWindow(unsigned int const width, unsigned int const height) const
    {
        return call(audioMasterSizeWindow, width, height) != 0;
    }
    bool updateDisplay() const;

    bool openFileSelector(::VstFileSelect const &fileSelectInfo) const
    {
        return call(audioMasterOpenFileSelector, 0, 0,
                    &const_cast<::VstFileSelect &>(fileSelectInfo)) != 0;
    }
    bool closeFileSelector(::VstFileSelect const &fileSelectInfo) const
    {
        return call(audioMasterCloseFileSelector, 0, 0,
                    &const_cast<::VstFileSelect &>(fileSelectInfo)) != 0;
    }

  public: // Offline functionality (http://www.kvraudio.com/forum/viewtopic.php?t=369997).
    bool offlineRead(VstOfflineTask const &offlineTask, VstOfflineOption const option,
                     bool const readSource) const
    {
        return call(audioMasterOfflineRead, readSource, option,
                    const_cast<VstOfflineTask *>(&offlineTask)) != 0;
    }
    bool offlineWrite(VstOfflineTask const &offlineTask, VstOfflineOption const option) const
    {
        return call(audioMasterOfflineWrite, 0, option,
                    const_cast<VstOfflineTask *>(&offlineTask)) != 0;
    }
    bool offlineStart(VstAudioFile &audioFiles, VstInt32 const numAudioFiles,
                      VstInt32 const numNewAudioFiles) const
    {
        return call(audioMasterOfflineStart, numNewAudioFiles, numAudioFiles, &audioFiles) != 0;
    }
    unsigned int offlineGetCurrentPass() const
    {
        return call(audioMasterOfflineGetCurrentPass) != 0;
    }
    unsigned int offlineGetCurrentMetaPass() const
    {
        return call(audioMasterOfflineGetCurrentMetaPass) != 0;
    }
  ...
  public: // Vendor specific.
    ...
    {
        return static_cast<unsigned int>(call(audioMasterGetVendorVersion));
    }

    VstIntPtr hostVendorSpecific(VstInt32 const lArg1, VstIntPtr const lArg2, void *const ptrArg,
                                 float const floatArg) const
    {
        return call(audioMasterVendorSpecific, lArg1, lArg2, ptrArg, floatArg);
    }

    bool isHost(char const *hostName) const;

  protected:
    static VSTHost24Proxy *fromAEffect(AEffect *const pEffect)
    {
        return static_cast<VSTHost24Proxy *>(AEffectWrapper::fromAEffect(pEffect));
    }
  ...
  private:
    ...
    {
        return call(opCode, index, value, ptr, 0);
    }
    VstIntPtr LE_COLD call(VstInt32 const opCode, VstInt32 const index, VstIntPtr const value) const
    {
        return call(opCode, index, value, 0, 0);
    }
    VstIntPtr LE_COLD call(VstInt32 const opCode, VstInt32 const index) const
    {
        return call(opCode, index, 0, 0, 0);
    }
    VstIntPtr LE_COLD call(VstInt32 const opCode) const { return call(opCode, 0, 0, 0, 0); }

    template <HostCapability capability> static char const *capabilityString()
    {
        switch (capability)
        {
        case SendVstEvents:
            return "sendVstEvents";
        case SendMidiEvent:
            return "sendVstMidiEvent";
        case SendTimeInfo:
            return "sendVstTimeInfo";
        case ReceiveVstEvents:
            return "receiveVstEvents";
        case ReceiveMidiEvent:
            return "receiveVstMidiEvent";
        case ReportConnectionChanges:
            return "reportConnectionChanges";
        case AcceptIOChanges:
            return "acceptIOChanges";
        case SizeWindow:
            return "sizeWindow";
        case Offline:
            return "offline";
        ...
        case StartStopProcess:
            return "startStopProcess";
        case ShellCategory:
            return "shellCategory";
        case SendVstMidiEventFlagIsRealtime:
            return "sendVstMidiEventFlagIsRealtime";
...
template <> struct ErrorCode<Protocol::VST24>
{
    enum value_type
    {
        Success = true,
...
template <> struct AutomatedParameterFor<Protocol::VST24>
{
    typedef NormalisedAutomatedParameter type;
};

class VSTPluginBase : protected VSTHost24Proxy,
                      public ErrorCode<Protocol::VST24> //...mrmlj...
{
  ...
  protected:
    VSTPluginBase(ConstructionParameter const constructionParameter)
        : VSTHost24Proxy(constructionParameter), pChunkData_(nullptr),
          pSpeakerArrangementStorage_(nullptr), allParametersInspected_(false), rect_(ERect())
    {
    }
    ~VSTPluginBase()
    {
        std::free(pChunkData_);
        std::free(pSpeakerArrangementStorage_);
    }
    ...
    static VSTPluginBase *fromAEffect(AEffect *const pEffect)
    {
        return static_cast<VSTPluginBase *>(VSTHost24Proxy::fromAEffect(pEffect));
    }
...
template <> class ParameterInformation<Protocol::VST24> : public ::VstParameterProperties
{
  public:
    ...
    void set(AutomatedParameter::Info const &info) LE_COLD
    {
        ...
        case AutomatedParameter::Info::FloatingPoint:
            ...
            if ((info.type != AutomatedParameter::Info::FloatingPoint) ||
                ((minInteger == info.minimum) && (maxInteger == info.maximum) &&
    ...
    void clear() { /*...mrmlj...broken Live undo system quick-fix search...flags = 0;*/
                   Utility::clear(static_cast<::VstParameterProperties &>(*this)); }

  private:
    ParameterInformation(ParameterInformation const &);

    template <class Parameter> void setValues(std::true_type /*is float*/)
    {
        ...
        if ((Parameter::rangeValuesDenominator == 1) &&
            ((Parameter::unscaledMinimum != 0) || (Parameter::unscaledMaximum != 1)))
        {
            flags |= kVstParameterUsesIntegerMinMax;
    ...
    template <class Parameter> void setValues(std::false_type /*is integral*/)
    {
        ...
        else if (!std::is_same<typename Parameter::Tag,
                               Parameters::PowerOfTwoParameterTag>::value &&
...
template <class ImplParam> class Plugin<ImplParam, Protocol::VST24> : public VSTPluginBase
{
  protected:
    ...
    class HostProxy : public VSTPluginBase::HostProxy
    {
      public:
        ...
        void LE_NOTHROW LE_COLD presetChangeEnd() const
        {
            ...
            {
                LE_TRACE("\tSW VST2.4: host rejected updateDisplay() (have to manually update all "
                         "parameters).");
        ...
        bool wantsManualDependentParameterNotifications() const
        {
            return !static_cast<Plugin const &>(
                        static_cast<VSTPluginBase::HostProxy const &>(*this))
                        .impl()
                        .gui();
        }
    }; // class HostProxy
#pragma warning(pop)
  ...
  protected:             // Default implementations for VST 2.4 host -> plugin
    ...
    static void getParameterLabel(ParameterIndex, ParameterStringBuf &label, Plugin const *)
    {
        label[0] = '\0';
    }
    void getParameterDisplay(ParameterIndex, ParameterStringBuf &text) const { text[0] = '\0'; }
    static void getParameterName(ParameterIndex, ParameterStringBuf &name, Plugin const *)
    {
        name[0] = '\0';
    }
    ...
    bool loadProgramState(unsigned int programIndex, char const *pProgramName, void const *pData,
                          unsigned int dataSize);
    unsigned int saveProgramState(unsigned int programIndex, void *pData,
    ...
    unsigned int getMidiProgramName(unsigned int /*channel*/, ::MidiProgramName &) const
    {
        return 0;
    }
    unsigned int getMidiProgramCategory(unsigned int /*channel*/, ::MidiProgramCategory &) const
    {
        return 0;
    }
    ...
    ::VstInt32 pluginSpecific(::VstInt32 /*index*/, ::VstIntPtr /*integerParam*/, void * /*pData*/,
                              float /*floatParam*/)
    {
        return 0;
    }

    bool offlineNotify(::VstAudioFile const &, unsigned int /*numAudioFiles*/, bool /*start*/)
    {
        return false;
    }
  ...
  private: // Private metafunctions.
    ...
    template <class Implementation> class HasProcessDoubleReplacing
    {
      private:
        typedef char no;
        ...
        template <class T>
        static yes sfinaeTester(
            SFINAEHelper<void (T::*)(double const *const *, double **, size_t), &T::process> *);
        template <class T> static no sfinaeTester(...);
...
#define LE_MA __attribute__((__may_alias__))
    ...
    template <typename T> union LE_MA AliasedType
    {
        T LE_MA value;
        char LE_MA aliasRuleRelaxer0;
        char LE_MA aliasRuleRelaxer1[sizeof(value)];

        constexpr AliasedType LE_MA &operator=(T const other)
        {
            value = other;
            return *this;
        }
    ...
    template <typename Target> struct AliasRuleBender
    {
        typedef AliasedType<Target> LE_MA type;
    };
    template <typename Target> struct AliasRuleBender<Target const>
    {
        typedef AliasedType<Target> LE_MA type;
    };
    template <unsigned int N> struct AliasRuleBender<char[N]>
    {
        typedef char type[N];
    };
    template <unsigned int N> struct AliasRuleBender<char const[N]>
    {
        typedef char const type[N];
    };

    template <typename Target>
    static typename AliasRuleBender<Target>::type *castTo(void *const source)
    {
        return static_cast<typename AliasRuleBender<Target>::type *>(source);
    }

    template <typename Target>
    static typename AliasRuleBender<Target>::type &castToReference(void *const source)
    {
        return *castTo<Target>(source);
    }

    template <typename Target>
    static typename AliasRuleBender<Target>::type &castToReference(std::intptr_t const source)
    {
        return castToReference<Target>(reinterpret_cast<void *>(source));
    }

    template <typename Target> static Target const &castToReference(void const *const source)
    {
...
#define LE_MA
    ...
    static AEffectProcessDoubleProc
    getDoubleReplacingCallback(boost::mpl::true_ /* has double replacing */)
    {
        return static_cast<AEffectProcessDoubleProc>(&Plugin::processReplacing);
    }
    static AEffectProcessDoubleProc
    getDoubleReplacingCallback(boost::mpl::false_ /* does not have double replacing */)
    {
        return 0;
    }
