...
{
    SW::GUI::warningMessageBox("SpectrumWorx unrecoverable failure...",
                               "Your host is about to crash :(", true);
    std::abort();
}
} // namespace LE
...
LE_ENTRY_POINT_BEGIN(AEffect *, VSTPluginMain, audioMasterCallback const audioMaster)
{
    ...
    unsigned int const hostSupportedVSTVersion(
        static_cast<unsigned int>(audioMaster(0, audioMasterVersion, 0, 0, 0, 0)));
    // Implementation note:
...
#if defined(_DEBUG)
    else if (hostSupportedVSTVersion < 2000)
    {
        LE_TRACE(
            "\tSW VST2.4: host application reported a bogus supported VST protocol version: %u.",
            hostSupportedVSTVersion);
    }
    else if (hostSupportedVSTVersion < 2400)
    {
        LE_TRACE("\tSW VST2.4: host application may not fully support the VST %.1f protocol, "
    ...
    {
        LE::SW::GUI::warningMessageBox("Failed to instantiate SpectrumWorx.",
                                       "Unknown unhandled error occurred!", true);
    }
...
#else
#define LE_PLUGIN_VST24_ENTRY_POINT(implClass)                                                     \
    extern "C" LE_ENTRY_POINT_ATTRIBUTES AEffect *__cdecl LE_PLUGIN_VST24_ENTRY_POINT_NAME(        \
        audioMasterCallback const audioMaster)                                                     \
    {                                                                                              \
...
#ifndef NDEBUG
template <class Impl> unsigned int Plugin<Impl, Protocol::VST24>::pluginInstanceCount_(0);
#endif
...
#endif
{
    ...
    aEffect().processDoubleReplacing =
        getDoubleReplacingCallback(typename HasProcessDoubleReplacing<Impl>::type());
    ...
    aEffect().flags = (effFlagsHasEditor) | (effFlagsProgramChunks) |
                      ...
                      (DECLARE_VST_DEPRECATED(effFlagsCanMono) *
                       (queryImplementationCapability<Ch1in2out>() == Can));
...
#ifndef NDEBUG
template <class Impl> Plugin<Impl, Protocol::VST24>::~Plugin()
{
                                                      ...
                                                      void *const pData, float const floatParam)
{
    ...
    case effSetSampleRate:
        impl.setSampleRate(floatParam);
        break;
    case effSetBlockSize:
        impl.setBlockSize(static_cast<unsigned int>(integerParam));
        break;

    case effStartProcess:
        return impl.preStreamingInitialization();
    case effStopProcess:
        return impl.postStreamingCleanup();
    // http://www.kvraudio.com/forum/viewtopic.php?t=148304
    case effMainsChanged:
        std::free(impl.pChunkData_);
        impl.pChunkData_ = nullptr;
        if (integerParam)
            impl.resume();
        else
            impl.suspend();
    ...
    case effGetChunk:
    {
        ...
        std::uint32_t const totalAllocationSize(Impl::maxNumberOfPrograms *
                                                (maximumProgramSize + kVstMaxProgNameLen));
        ...
        LE_TRACE_IF(impl.pChunkData_ && impl.pChunkData_ != pChunkData,
                    "\tSW VST2.4: chunk storage relocated.");
        impl.pChunkData_ = pChunkData;

        char *pCurrentPosition(static_cast<char *>(pChunkData));

        castToReference<std::uint32_t>(pCurrentPosition) = Impl::vstUniqueID;
        pCurrentPosition += sizeof(std::uint32_t);
        castToReference<std::uint32_t>(pCurrentPosition) = Impl::version;
        pCurrentPosition += sizeof(std::uint32_t);
        castToReference<std::uint32_t>(pCurrentPosition) = programsToGet;
        pCurrentPosition += sizeof(std::uint32_t);
        ...
        for (auto programIndex(startProgramIndex); programIndex < endProgramIndex; ++programIndex)
        {
            ...
            std::uint32_t const actualProgramSize(
                impl.saveProgramState(programIndex, pCurrentPosition, maximumProgramSize));
            pCurrentPosition += actualProgramSize;
        ...
        LE_TRACE_IF(pShrunkChunkData == impl.pChunkData_,
                    ...
                    totalAllocationSize, actualStorageUsed);
    ...
    case effSetChunk:
    {
        ...
        auto const id(castToReference<std::uint32_t const>(pCurrentPosition));
        pCurrentPosition += sizeof(id);
        ...
        std::uint32_t const version(castToReference<std::uint32_t const>(pCurrentPosition));
        pCurrentPosition += sizeof(version);
        if (version < 2800) //...mrmlj...pre 2.8 SW hardcoded handling...
        {
            ...
            auto const activeProgram(castToReference<std::uint32_t const>(pCurrentPosition));
        ...
        else
        {
            ...
            for (std::uint8_t programIndex(startProgramIndex); programIndex < endProgramIndex;
                 ++programIndex)
            {
                auto const programNameSize(castToReference<std::uint32_t const>(pCurrentPosition));
                pCurrentPosition += sizeof(programNameSize);
                char const *const pProgramName(pCurrentPosition);
                pCurrentPosition += programNameSize;
    ...
    case effGetParameterProperties:
        return Impl::getParameterProperties(
            parameterIndex,
            static_cast<ParameterInformation &>(static_cast<::VstParameterProperties &>(
                castToReference<::VstParameterProperties>(pData))),
            pContextForDynamicParameterAccess);
    ...
    case effBeginSetProgram:
        return impl.beginSetProgram();
    case effEndSetProgram:
        return impl.endSetProgram();

    case effBeginLoadBank:
        return impl.beginLoadBank(castToReference<::VstPatchChunkInfo const>(pData));
    case effBeginLoadProgram:
        return impl.beginLoadProgram(castToReference<::VstPatchChunkInfo const>(pData));
    ...
    case effGetInputProperties:
        if (unsigned(index) < impl.host().getNumInputs())
        {
            impl.getInputProperties(index, castToReference<::VstPinProperties>(pData));
            return true;
        }
        else
            return false;
    case effGetOutputProperties:
        if (unsigned(index) < impl.host().getNumOutputs())
        {
            impl.getOutputProperties(index, castToReference<::VstPinProperties>(pData));
            return true;
        }
        else
            return false;
    ...
    {
        ::VstSpeakerArrangement const &inputArrangement(
            castToReference<::VstSpeakerArrangement const>(integerParam));
        ::VstSpeakerArrangement const &outputArrangement(
            castToReference<::VstSpeakerArrangement const>(pData));
    ...
    {
        ::VstSpeakerArrangement *LE_RESTRICT &pInputArrangement =
            castToReference<::VstSpeakerArrangement *>(integerParam);
        ::VstSpeakerArrangement *LE_RESTRICT &pOutputArrangement =
            castToReference<::VstSpeakerArrangement *>(pData);
        ...
        unsigned int const inputArrangementStorage(
            sizeof(*pInputArrangement) + (inputChannels - preallocatedSpeakers) * speakerStorage);
        unsigned int const outputArrangementStorage(
            sizeof(*pOutputArrangement) + (outputChannels - preallocatedSpeakers) * speakerStorage);
        unsigned int const totalArrangementStorage(inputArrangementStorage +
                                                   outputArrangementStorage);
        unsigned char *LE_RESTRICT const pArrangementStorage(static_cast<unsigned char *>(
        ...
        LE_TRACE_IF(impl.pSpeakerArrangementStorage_ &&
                    ...
                    "\tSW VST2.4: speaker arrangement storage relocated.");
        impl.pSpeakerArrangementStorage_ = pArrangementStorage;
        ...
        pOutputArrangement = &reinterpret_cast<::VstSpeakerArrangement &>(
    ...
    case effSetPanLaw:
        return impl.setPanLaw(static_cast<::VstPanLawType>(integerParam), floatParam);

    case effSetBypass:
        return impl.setBypass(integerParam != 0);

    case effSetProcessPrecision:
        return impl.setProcessPrecision(static_cast<::VstProcessPrecision>(integerParam));
    ...
    case effEditKeyDown:
        ...
        if (!impl.gui())
            return false; // Check if this is part of the cleanup message pump loop (see the note in ~ReferenceCountedGUIInitializationGuard())...
#endif                    // OSX x86
        return impl.gui()->onKeyDown(
            static_cast<char>(index), static_cast<VstVirtualKey>(integerParam),
    ...
    case effSetEditKnobMode:
        return impl.gui() ? impl.gui()->setKnobMode(static_cast<Editor::KnobMode>(integerParam))
                          : false;

    //---MIDI----------------------
    case effGetMidiProgramName:
        return impl.getMidiProgramName(index, castToReference<::MidiProgramName>(pData));
    case effGetCurrentMidiProgram:
        return impl.getCurrentMidiProgram(index, castToReference<::MidiProgramName>(pData));
    case effGetMidiProgramCategory:
        return impl.getMidiProgramCategory(index, castToReference<::MidiProgramCategory>(pData));
    case effHasMidiProgramsChanged:
        return impl.hasMidiProgramsChanged(index);
    case effGetMidiKeyName:
        return impl.getMidiKeyName(index, castToReference<::MidiKeyName>(pData));

    case effGetNumMidiInputChannels:
        return impl.getNumMidiInputChannels();
    case effGetNumMidiOutputChannels:
        return impl.getNumMidiOutputChannels();

    //---OfflineProcessing----------------------
    case effOfflineNotify:
        return impl.offlineNotify(castToReference<VstAudioFile const>(pData),
                                  static_cast<unsigned int>(integerParam), index != 0);
    case effOfflinePrepare:
        return impl.offlinePrepare(castToReference<VstOfflineTask const>(pData),
                                   static_cast<unsigned int>(integerParam));
    case effOfflineRun:
        return impl.offlineRun(castToReference<VstOfflineTask const>(pData),
                               static_cast<unsigned int>(integerParam));

    case effProcessVarIo:
        return impl.processVariableIo(static_cast<VstVariableIo const *>(pData));
    ...
    case effGetEffectName:
        std::strcpy(castToReference<EffectNameBuf>(pData), Impl::name);
        return true;
    case effGetVendorString:
        std::strcpy(castToReference<VendorStringBuf>(pData), "Little Endian");
        return true;
    case effGetProductString:
        std::strcpy(castToReference<ProductStringBuf>(pData), Impl::productString);
        return true;
    case effGetVendorVersion:
        return Impl::version;

    case effVendorSpecific:
        return impl.pluginSpecific(index, integerParam, pData, floatParam);
    ...
    case effCanDo:
    {
        ...
        if (capability == "sendVstEvents")
            return impl.template queryImplementationCapability<SendVSTEvents>();
        else if (capability == "sendVstMidiEvent")
            return impl.template queryImplementationCapability<SendMIDIEvent>();
        else if (capability == "receiveVstEvents")
            return impl.template queryImplementationCapability<ReceiveVSTEvents>();
        else if (capability == "receiveVstMidiEvent")
            return impl.template queryImplementationCapability<ReceiveMIDIEvent>();
        else if (capability == "receiveVstTimeInfo")
            return impl.template queryImplementationCapability<ReceiveVSTTimeInfo>();
        else if (capability == "offline")
            return impl.template queryImplementationCapability<OfflineProcessing>();
        else if (capability == "midiProgramNames")
            return impl.template queryImplementationCapability<MidiProgramNames>();
        else if (capability == "bypass")
            return impl.template queryImplementationCapability<Bypass>();
        else if (capability == "mixDryWet")
            return impl.template queryImplementationCapability<MixDryWet>();
        else if (capability == "noRealTime")
            return impl.template queryImplementationCapability<NonRealTimeProcessing>();
        else if (capability == "plugAsChannelInsert")
            return impl.template queryImplementationCapability<AsInsert>();
        else if (capability == "plugAsSend")
            return impl.template queryImplementationCapability<AsSend>();
        else if (capability == "1in1out")
            return impl.template queryImplementationCapability<Ch1in1out>();
        else if (capability == "1in2out")
            return impl.template queryImplementationCapability<Ch1in2out>();
        else if (capability == "2in1out")
            return impl.template queryImplementationCapability<Ch2in1out>();
        else if (capability == "2in2out")
            return impl.template queryImplementationCapability<Ch2in2out>();
        else if (capability == "2in4out")
            return impl.template queryImplementationCapability<Ch2in4out>();
        else if (capability == "4in2out")
            return impl.template queryImplementationCapability<Ch4in2out>();
        else if (capability == "4in4out")
            return impl.template queryImplementationCapability<Ch4in4out>();
        else if (capability == "4in8out")
            return impl.template queryImplementationCapability<Ch4in8out>();
        else if (capability == "8in4out")
            return impl.template queryImplementationCapability<Ch8in4out>();
        else if (capability == "8in8out")
            return impl.template queryImplementationCapability<Ch8in8out>();
        else if (capability == "conformsToWindowRules")
            return impl.template queryImplementationCapability<UsesAFixedSizeGUI>();
        ...
        {
#ifdef _DEBUG
            if (capability == "multipass" || capability == "metapass" ||
            ...
            {
                LE_TRACE(
                    "\tSW VST2.4: a 'known but not understood' canDo query was received (%s).\n",
                    capability.begin());
            }
            else
            {
    ...
    //---Parameters and Programs----------------------
    case DECLARE_VST_DEPRECATED(
        effGetNumProgramCategories): // http://homepage.hik.se/staff/tkama/audio_procME/doc/html/plug/2.0/AudioEffectX.html#getNumCategories
    case DECLARE_VST_DEPRECATED(
        effCopyProgram): // http://homepage.hik.se/staff/tkama/audio_procME/doc/html/plug/2.0/AudioEffectX.html#copyProgram
    //---Connections, Configuration----------------------
    ...
    case DECLARE_VST_DEPRECATED(effGetVu):
        ...
        LE_TRACE_IF(opCode != DECLARE_VST_DEPRECATED(effIdle),
                    "\tSW VST2.4: deprecated VST opcode invoked (opcode: %d, integer: %d, intptr: "
                    "%d, pointer: %p, float: %f).\n",
...
#ifdef _DEBUG
    default:
        LE_TRACE("\tSW VST2.4: unhandled VST opcode invoked (opcode: %d, integer: %d, intptr: %d, "
                 "pointer: %p, float: %f).\n",
                 opCode, index, integerParam, pData, floatParam);
                                                 ...
                                                 float const value) /// \throws nothing
{
    ...
    if (impl(pEffect).setParameter(parameterIndex, value) == ErrorCode::Success)
    {
        ...
        if (Impl::getParameterProperties(parameterIndex, static_cast<ParameterInformation &>(info),
...
VSTPluginBase::CanDoAnswer Plugin<Impl, Protocol::VST24>::queryImplementationCapability() const
{
    ...
    bool const isImplementationCapable(
        boost::mpl::has_key<typename Impl::Capabilities,
                            boost::mpl::integral_c<typename Impl::Capabilities::value_type,
                                                   pluginCapability>>::value);

    VSTPluginBase::CanDoAnswer const result(
        (isImplementationCapable || impl().template queryDynamicCapability<pluginCapability>())
            ? Can
            : Cannot);
    LE_TRACE("\t\tanswer: %d\n", result);
    return result;
...
template <class Impl> Impl &Plugin<Impl, Protocol::VST24>::impl() /// \throws nothing
{
