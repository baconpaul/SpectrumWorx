...
bool VSTHost24Proxy::reportNewNumberOfIOChannels(unsigned int const inputs,
                                                 unsigned int const sideInputs,
                                                 unsigned int const outputs)
{
...
#ifndef NDEBUG
    auto const maxChannels(
        reinterpret_cast<std::pair<unsigned short, unsigned short> const &>(aEffect().user));
    LE_TRACE_IF(newInputs > maxChannels.first,
                "\tSW VST2.4: enabling more input channels than the initially reported maximum.");
    LE_TRACE_IF(newOutputs > maxChannels.second,
                "\tSW VST2.4: enabling more output channels than the initially reported maximum.");
#endif // NDEBUG
...
bool VSTHost24Proxy::reportNewLatencyInSamples(unsigned int const samples)
{
    ...
    LE_TRACE_IF(
        unsigned(aEffect().initialDelay) != samples,
        "\tSW VST2.4: Avoid calling back the host if not necessary (latency change notification).");
...
void LE_NOTHROW VSTHost24Proxy::allParametersChanged() const
{
    ...
    for (ParameterIndex parameterIndex{0}; parameterIndex.value < numberOfParameters;
    ...
    for (ParameterIndex parameterIndex{0}; parameterIndex.value < numberOfParameters;
         ++parameterIndex.value)
    {
        AutomatedParameterValue const parameterValue(
            getParameter(const_cast<::AEffect *>(&effect), parameterIndex.value));
