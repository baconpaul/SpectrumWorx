...
#ifndef NDEBUG
template <class Impl> unsigned int Plugin<Impl, Protocol::Unity>::pluginInstanceCount_(0);
#endif
...
Plugin<Impl, Protocol::Unity>::create(UnityAudioEffectState *LE_RESTRICT const pState)
{
    ...
    if (pImpl.get() && Impl::makeErrorCode(pImpl->setSampleRate(pState->samplerate)) == Success &&
    ...
    int const outChannels)
{
    ...
    auto const numberOfSideChannels(
        static_cast<std::uint8_t>(pState->sidechainbuffer ? numberOfMainChannels : 0));

    LE_TRACE_IF(
        (numberOfMainChannels != impl.numberOfInputChannels()) ||
