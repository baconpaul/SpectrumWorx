...
@interface LE_PLUGIN_AU_VIEW : NSView
{
    ::AudioUnit effect_;
}
...
- initWithFrame:(NSRect)frame lePlugin:(::AudioUnit)audioUnit
{
    ...
    if (self)
    {
        ...
        if (AudioUnitSetProperty(audioUnit, LE::Plugins::Detail::auViewPropertyID,
                                 kAudioUnitScope_Global, 0, &pParentView,
                                 sizeof(pParentView)) != noErr)
            return nil;
...
- (void)close
{
    ...
    LE_TRACE_IF(result != noErr,
                "\t SW AU: LEPluginAUView failed (%ld) to close the effect GUI (maybe the effect "
