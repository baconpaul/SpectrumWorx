...
class AudioBuffers
{
  public:
    ...
    std::uint8_t numberOfBuffers() const;
    std::uint8_t numberOfChannels() const;
    std::uint16_t numberOfSamples() const;
...
template <typename T, unsigned int N> class VariableLengthArray : public std::array<T, N>
{
  public:
...
class AUTimingInformation
{
  public:
    enum FieldFlags
    {
        ...
        NumberOfBeats =
            kAudioTimeStampHostTimeValid, // http://developer.apple.com/library/ios/#qa/qa1643/_index.html, http://www.macresearch.org/tutorial_performance_and_time
  ...
  public:
    ...
    template <FieldFlags desiredField> bool hasField() const { return fields_ & desiredField; }

    template <unsigned int desiredFields> bool hasFields() const
    {
        return (fields_ & desiredFields) == desiredFields;
    }

  private:
    friend class Plugins::AUHostProxy;
    ...
    template <unsigned int filterMask> struct ConditionalData
    {
        ConditionalData() : flags(0) {}

        template <unsigned int fields> static bool wants() { return filterMask & fields; }

        template <FieldFlags field, typename T> static T *get(T &data)
        {
            return (filterMask & field) ? &data : nullptr;
        }

        template <unsigned int fields> void setFlags(OSStatus const result)
        {
            if (result == noErr)
            {
                flags |= fields & filterMask;
            }
            else
            {
...
AUTimingInformation
AUTimingInformation::create(::HostCallbackInfo const &LE_RESTRICT callbacks,
                            ::AudioTimeStamp const *LE_RESTRICT const pCurrentTimeStamp)
{
    ...
    {
        ::OSStatus const result((*callbacks.beatAndTempoProc)(
            callbacks.hostUserData, data.template get<NumberOfBeats>(info.currentBeat_),
            data.template get<BPM>(info.currentTempo_)));
        LE_TRACE_IF(result != noErr,
                    "\tSW AU: beatAndTempo host callback failed (" LE_OSX_INT_FORMAT(d) ")",
                    result);
    ...
    {
        ::OSStatus const result((*callbacks.musicalTimeLocationProc)(
        ...
        LE_TRACE_IF(result != noErr,
                    "\tSW AU: musicalTimeLocation host callback failed (" LE_OSX_INT_FORMAT(d) ")",
                    result);
        data.template setFlags<LastBarPosition | TimeSignature>(result);
    }
    if (data.template wants<SampleTime | TransportState | CyclePosition>() &&
        callbacks.transportStateProc)
    {
        ::OSStatus const result((*callbacks.transportStateProc)(
            callbacks.hostUserData, data.template get<TransportState>(info.isPlaying_),
        ...
        LE_TRACE_IF(result != noErr,
                    "\tSW AU: transportState host callback failed (" LE_OSX_INT_FORMAT(d) ")",
                    result);
        data.template setFlags<SampleTime | TransportState | CyclePosition>(result);
        if (data.template wants<SampleTime>() && !(data.flags & SampleTime) && pCurrentTimeStamp)
        {
...
class AUHostProxy
{
  ...
  public: // Host capabilities.
    ...
    template <HostCapability capability> bool canDo() const { return false; }

  public: // Host information/state.
    template <unsigned int filterMask> Detail::AUTimingInformation getTimeInfo() const
    {
        return Detail::AUTimingInformation::create<filterMask>(callbacks(), pCurrentTimeStamp_);
    }
  ...
  public: // Plugin information/state reported/visible to host.
    ...
    bool reportNewNumberOfIOChannels(std::uint8_t inputs, std::uint8_t sideInputs,
                                     std::uint8_t outputs) const;
  ...
  public:
    OSStatus addPropertyListener(::AudioUnitPropertyID, ::AudioUnitPropertyListenerProc,
                                 void *pUserData);
    OSStatus removePropertyListener(::AudioUnitPropertyID, ::AudioUnitPropertyListenerProc,
  ...
  protected:
    ...
    void setCurrentTimeStamp(::AudioTimeStamp const *LE_RESTRICT pCurrentTimeStamp)
    {
        pCurrentTimeStamp_ = pCurrentTimeStamp;
    }
...
template <> struct ErrorCode<Protocol::AU>
{
    enum value_type
    {
        Success = noErr,
...
template <> struct AutomatedParameterFor<Protocol::AU>
{
    typedef FullRangeAutomatedParameter type;
};
    ...
    /*private...mrmlj...*/ public AUHostProxy
{
  ...
  protected:
    ...
    void sendRenderNotification(::AudioUnitRenderActionFlags extraFlags,
                                ::AudioUnitRenderActionFlags &, ::AudioTimeStamp const &,
                                ::UInt32 busNumber, ::UInt32 numberFrames,
                                ::AudioBufferList &) const;

    void setRenderResult(OSStatus renderResult);
  ...
  protected:
    ...
    class RenderDelegate
    {
      public:
        ...
        OSStatus operator()(::AudioUnitRenderActionFlags &, ::AudioTimeStamp const &,
                            ::UInt32 numberFrames, ::AudioBufferList &) const;

        void
        operator=(std::pair<::AURenderCallbackStruct const &, ::AudioUnitElement> renderCallback);
    ...
    class InputConnection : public RenderDelegate
    {
      public:
        ...
        OSStatus operator()(::AudioUnitRenderActionFlags &, ::AudioTimeStamp const *,
                            ::UInt32 numberFrames, ::AudioBufferList *,
                            ::UInt32 numberOfChannels) const;

        LE_ALIGN(16)::AudioBufferList &LE_RESTRICT buffers()
        {
            return *static_cast<::AudioBufferList *>(buffers_);
        }
        LE_ALIGN(16)::AudioBufferList const &LE_RESTRICT buffers() const
        {
            return const_cast<InputConnection &>(*this).buffers();
        }
...
template <> class ParameterInformation<Protocol::AU> : public ::AudioUnitParameterInfo
{
  public:
    ...
    template <class Parameter> result_type operator()() LE_COLD
    {
        ...
        {
            unit = std::is_same<value_type, bool>::value ? kAudioUnitParameterUnit_Boolean
                                                         : kAudioUnitParameterUnit_Indexed;
        }

        if ( //...mrmlj...for N/A parameters/'dynamic' interface...
    ...
    void markAsMeta() { flags |= kAudioUnitParameterFlag_IsElementMeta; }

  private:
    ParameterInformation(ParameterInformation const &);

    template <class Parameter> void setValues(std::true_type /*is power-of-two*/)
    {
    ...
    template <class Parameter> void setValues(std::false_type /*is linear*/)
    {
...
template <class ImplParam> class Plugin<ImplParam, Protocol::AU> : public AUPluginBase
{
  protected:
  ...
  protected:
    ...
    {
        return Plugins::ErrorCode<Protocol::AU>::Success;
    } ///< VSTSDK AudioEffect::open () equivalent.
    ...
    void getParameterLabel(unsigned int /*index*/, ParameterStringBuf &label) const
    {
        label[0] = '\0';
    }
    void getParameterDisplay(unsigned int /*index*/, ParameterStringBuf &text) const
    {
        text[0] = '\0';
    }
    void getParameterName(unsigned int /*index*/, ParameterStringBuf &name) const
    {
        name[0] = '\0';
    }

    bool getParameterProperties(unsigned int /*index*/, ParameterInformation &) const
    {
        return false;
    }

    bool canParameterBeAutomated(unsigned int /*index*/, void const * /*pContext*/) const
    {
        return true;
    }

    std::size_t getChunk(void ** /*pData*/, bool /*isPreset*/) const { return 0; }
    std::size_t setChunk(void * /*pData*/, std::size_t /*byteSize*/, bool /*isPreset*/)
    {
        return 0;
    }
  ...
  private:
    template <typename T, typename Pointer> static T &castToReference(Pointer const pointer)
    {
  ...
  private:
    ...
    OSStatus checkSideChannelBusStateChange()
    {
        //...mrmlj...SW HARDCODE...
        auto &impl(this->impl());
        ...
        return Detail::makeErrorCode(
            impl.enableSideChannelInput(impl.host().sideChannels_ && impl.inputConnections_[1]));
    }
