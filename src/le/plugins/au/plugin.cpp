                                                     ...
                                                     ::AudioUnitElement const element) const
{
    ...
    {
        listener.callback(listener.pUserData, auInstance(), propertyID, scope, element);
    }
    return true;
}
...
{
    bool const parameterListChangeAcknowledged(
        globalPropertyChanged(kAudioUnitProperty_ParameterList));
...
void AUHostProxy::fireParameterEvent(::AudioUnitEventType const eventType,
                                     ::AudioUnitParameterID const parameterID) const
{
                                          ...
                                          void *const pUserData)
{
    ...
    if (pListeners)
    {
        ...
        for (auto const &listener : *pListeners)
        {
            LE_TRACE_IF((listener.callback == callback) && (listener.pUserData == pUserData),
                        "\tSW AU: adding a duplicate property (" LE_OSX_INT_FORMAT(u) ") listener.",
                        propertyID);
        }
#endif // _DEBUG
    ...
    LE_TRACE_IF(
        result == noErr,
        "\tSW AU: added listener (%X, %X) number %u for property (" LE_OSX_INT_FORMAT(u) ").",
        callback, pUserData, pListeners->size() - 1, propertyID);
    LE_TRACE_IF(
        result != noErr,
        "\tSW AU: failed to add listener (%X, %X) number %u for property (" LE_OSX_INT_FORMAT(
            u) ").",
        callback, pUserData, pListeners->size(), propertyID);
    return result;
}

OSStatus AUHostProxy::removePropertyListener(::AudioUnitPropertyID const propertyID,
                                             ::AudioUnitPropertyListenerProc const callback,
                                             void const *const pUserData)
{
    ...
    if (!pListeners)
    {
        LE_TRACE("\tSW AU: failed to remove listener (%X, %X) for property (" LE_OSX_INT_FORMAT(
                     u) ") - no listeners for that property.",
                 callback, pUserData, propertyID);
    ...
    PropertyChangeListener *LE_RESTRICT const pElement(
        std::find_if(pListeners->begin(), pListeners->end(),
                     [&, trailBytesToSkip](PropertyChangeListener const &other) {
                         return std::memcmp(&other, &propertyListener,
                                            sizeof(propertyListener) - trailBytesToSkip) == 0;
                     }));
    if (pElement == pListeners->end())
    {
        LE_TRACE("\tSW AU: failed to remove listener (%X, %X) for property (" LE_OSX_INT_FORMAT(
                     u) ") - listener not found.",
                 callback, pUserData, propertyID);
    ...
    LE_TRACE("\tSW AU: removed listener (%X, %X) for property (" LE_OSX_INT_FORMAT(u) ").",
             callback, pUserData, propertyID);
    return result;
}

AUHostProxy::PropertyListeners *__restrict__ AUHostProxy::findListenersFor(
...
AUHostProxy::PropertyListeners const *LE_RESTRICT
AUHostProxy::findListenersFor(::AudioUnitPropertyID const propertyID) const
{
...
bool AUHostProxy::reportNewNumberOfIOChannels(std::uint8_t const inputs,
...
bool AUHostProxy::reportNewLatencyInSamples(std::uint16_t /*samples*/) const
{
    return globalPropertyChanged(kAudioUnitProperty_Latency);
}
bool AUHostProxy::currentPresetNameChanged() const
{
    return globalPropertyChanged(kAudioUnitProperty_PresentPreset);
}
...
namespace Detail
{
OSStatus makeErrorCode(AUPluginBase::ErrorCode const errorCode)
{
    return static_cast<OSStatus>(errorCode);
}
...
bool AudioBuffers::resize(std::uint8_t const numberOfChannels, std::uint16_t const numberOfSamples,
                          bool const interleaved)
{
    ...
    /// it.
    ///                                   (12.11.2015.) (Domagoj Saric)
    if (pIOBuffers_ && numberOfChannels == this->numberOfChannels() &&
        numberOfSamples == this->numberOfSamples())
        return true;

    auto const structureRequiredStorage(sizeof(*pIOBuffers_) +
                                        ((numberOfBuffers - 1) * sizeof(*pIOBuffers_->mBuffers)));
    auto const structureAlignmentPadding(16 - (structureRequiredStorage % 16));
    auto const perBufferRequiredStorage(numberOfSamples * sizeof(float) *
                                        numberOfChannelsPerBuffer);
...
bool AudioBuffers::interleaved() const { return bufferList().mBuffers[0].mNumberChannels != 1; }

std::uint8_t AudioBuffers::numberOfBuffers() const
{
    return static_cast<std::uint8_t>(bufferList().mNumberBuffers);
}

std::uint8_t AudioBuffers::numberOfChannels() const
{
    return std::max<std::uint8_t>(
        numberOfBuffers(), static_cast<std::uint8_t>(bufferList().mBuffers[0].mNumberChannels));
}

std::uint16_t AudioBuffers::numberOfSamples() const
{
    auto const &buffer(bufferList().mBuffers[0]);
    return static_cast<std::uint16_t>(buffer.mDataByteSize / sizeof(float) /
...
#ifndef NDEBUG
    std::memset(this, 0,
                reinterpret_cast<char const *>(&this->fields_) -
                    reinterpret_cast<char const *>(this));
#endif // NDEBUG
}
} // namespace Detail

AUPluginBase::AUPluginBase(::AudioUnit const auInstance) : AUHostProxy(auInstance)
{
...
void AUPluginBase::sendRenderNotification(::AudioUnitRenderActionFlags const extraFlags,
...
OSStatus AUPluginBase::RenderDelegate::operator()(::AudioUnitRenderActionFlags &actionFlags,
                                                  ::AudioTimeStamp const &timeStamp,
                                                  ::UInt32 const numberFrames,
                                                  ::AudioBufferList &buffers) const
{
    ...
    return renderCallback_.inputProc(renderCallback_.inputProcRefCon, &actionFlags, &timeStamp,
                                     bus_, numberFrames, &buffers);
...
void AUPluginBase::RenderDelegate::operator=(
    std::pair<::AURenderCallbackStruct const &, ::AudioUnitElement> const renderCallback)
{
    ...
    ::AudioBufferList *LE_RESTRICT const pExternalBuffers, ::UInt32 const numberOfChannels) const
{
    ...
    OSStatus const result(
        RenderDelegate::operator()(ioActionFlags, *pTimeStamp, numberFrames, buffers_));
    return result;
}
