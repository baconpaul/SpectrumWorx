...
Comparison compare(Digest const &golden, Digest const &actual, bool const exact)
{
    ...
    if (actual.nonFiniteSamples != golden.nonFiniteSamples)
    ...
    if (std::abs(golden.dcOffset - actual.dcOffset) > tolerance * std::max(golden.rms, 1e-6f))
    ...
    for (unsigned int band(0); band < numberOfBands; ++band)
    {
        ...
        if ((golden.bands[band] <= silenceFloor + 1) && (actual.bands[band] <= silenceFloor + 1))
            continue;
        constexpr float bandTolerance{0.01f}; // dB
        if (std::abs(golden.bands[band] - actual.bands[band]) > bandTolerance)
            return fail("band " + std::to_string(band) + " " + std::to_string(actual.bands[band]) +
                        " dB, expected " + std::to_string(golden.bands[band]) + " dB");
    }
