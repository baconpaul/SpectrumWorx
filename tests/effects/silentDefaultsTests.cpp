...
/// -----------------------
///
///   **25 of the 464 golden fixtures render pure silence**, which is a fixture
...
///     - **It is not the reason anyway.** Each of the three effects is silent
///       for a reason of its own, none of them about external audio, and all
///       three are ordinary parameter defaults:
///
...
///   | `Frecho`, `Frevcho` | `Distance` defaults to 100 m, and the delay is the round trip at 343 m/s: **583 ms**. The fixture is 16384 frames at 44100, which is **371 ms**. The echo has not come back yet. |
...
///
///   So what this file does is state each of those, both ways round -- silent at
/// the default, audible with the one parameter moved -- and then hold the count
/// of silent fixtures in `goldens.txt` to what it is, so it can only fall.
///
/// \note Rendering the *fix* rather than fixing the defaults. All four are 2016
/// behaviour and changing any of them changes what a preset sounds like, which
...
/// The golden matrix's own smaller configuration and its own length, because
/// what is being explained is those fixtures and not a rendering of this file's
/// choosing. 16384 frames at 44100 is 371 ms.
constexpr SWTest::RenderSetup setup{512, 4, 2, 44100, 256};
constexpr std::uint32_t frames{16384};
             ...
             SWTest::RenderSetup const &configuration = setup)
{
    ...
    return peak(SWTest::renderChain(configuration, slots, signal, frames));
}
} // anonymous namespace
//------------------------------------------------------------------------------
...
TEST_CASE("Frecho is silent because the echo has not come back yet", "[effects][silent-defaults]")
{
    ////////////////////////////////////////////////////////////////////////////
    ///
    /// \note Arithmetic rather than a guess: the delay is
    /// `Distance * 2 * 1000 / 343` milliseconds (`frechoImpl.cpp:64`), so the
    /// 100 m default is a 583 ms round trip against a 371 ms fixture. 17 m is
    /// the parameter's minimum and puts it at 99 ms, comfortably inside.
    ///
    ///   Which makes these sixteen fixtures a statement about the *fixture
    /// length*, not about the effect. A golden matrix rendering two seconds
    ...
    /// \note And the boundary, which is what says the reading is the *delay*
    /// rather than "small numbers work": the fixture is 371 ms, so a distance
    /// whose round trip is just under that sounds and one just over it does not.
    /// 60 m is 350 ms; 70 m is 408 ms.
    CHECK(peakOf("Frecho", Signal::PinkNoise,
                 [](Module &module) { module.setEffectParameter(distance, 60); }) > 0.0f);
    CHECK(peakOf("Frecho", Signal::PinkNoise,
                 [](Module &module) { module.setEffectParameter(distance, 70); }) == 0.0f);
}
          ...
          "[effects][silent-defaults]")
{
    ...
    /// 25, they are explained above, and this is what keeps that number from
    ...
    /// The four families this file explains, and nothing else. Should a
    /// default change and take a row out of the set, this is the line to lower.
    CHECK(silent.size() == 25);
