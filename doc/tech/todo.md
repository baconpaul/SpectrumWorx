...
| | |
|---|---|
| Builds | CLAP, VST3, AUv2, standalone — macOS arm64. Linux arm64 last proven at stage 4. Windows arrives as logs. |
...
| Tests | **264/264** as of 04.08.2026, in both build trees. Two binaries, `sw-dsp-tests` and `sw-plugin-tests`. Goldens run in Release only. |
...
| Warnings | **229 unique sites**, 227 of them one class — item 3. Everything else our targets emitted is fixed. |
...
| 3 | **The stage 7 tail** — the 227 `-Wundefined-var-template`, seven macros no build can define, include-what-you-use | 3–4 days |
| 4 | **Ship** — README, manual, installers, notarisation | 1–2 weeks |
...
both and narrow the gap without closing it.

### 3 — The stage 7 tail, and the macros that make live code lie

Three things, one pass, one judgement.

**The 227 `-Wundefined-var-template`** — 86 % of every warning the tree emits,
and one cause. `Parameters::Name<Parameter>` declares `static char const
string_[]` with no definition (`uiElements.hpp:62-65`); each parameter's name is
supplied by an explicit specialisation emitted by `EFFECT_PARAMETER_NAME` into a
`.cpp`. Clang cannot see the definition where `name()` instantiates, so it warns
— and it is right to: this is the *exact* shape of the release-only link error
stage 5 found in `ModuleParameters::parameterInfos()`. It links today because
every specialisation happens to be reachable; nothing enforces that.

The tree already contains the fix, applied to five parameters and no more
(`baseParametersUIElements.hpp:53-59`), and it is also the proof of what the fix
has to look like: those five **still warn** from `plugin2Host.cpp:599`, which
instantiates `name<BaseParameters::Bypass>()` and does not include that header.
**So the declaration has to travel with the parameter** — either
`LE_DEFINE_PARAMETER` emits it, or `EFFECT_PARAMETER_NAME` gains a companion each
effect's own header uses. It is a change to the parameter system
(`parameter_system.md`), which is why it is here rather than behind
`-Wno-undefined-var-template`: the warning is currently the only thing checking
that a parameter's name exists at all.

**Seven feature macros are tested by `#if` in compiled translation units and
defined nowhere a live build can reach.** Two of them are defined *only* in
`src/le/spectrumworx/engine/configuration.cmake`, reachable only from
...
| `LE_SW_SDK_BUILD` | `musicalScales`, `presets.{hpp,cpp}`, `module.cpp`, `tuneWorxImpl.cpp` | Unconditionally `namespace X {`. |
| `LE_SW_FMOD` | ~13, incl. `spectrumWorxCore.cpp`, `plugin2Host.cpp`, `spectrumWorxEditor.cpp` | The FMOD backend was deleted in stage 0. |
| `LE_SW_FULL` | `factory.cpp:160`, `presets.cpp:525`, `moduleMenuHolder.cpp:45` | The paid/demo SKU gate. |
...
`src/le/utility/CMakeLists.txt`) should carry the same "record, not build" banner
`legacy-build.cmake` has.

Careful with one neighbour: **`LE_HAS_NT2` / `LE_MATH_USE_NT2`** account for
roughly **534 lines across 33 regions of `le/math/vector.cpp`**, about a quarter
of the file, that cannot be reached now stage 4 is done — but `math.cpp:414` and
`conversion.cpp:83` spell it `#if defined(__APPLE__) || !defined(LE_HAS_NT2)`,
and those are the **live** arms.

**Include-what-you-use, and the force-include.**
`leConfigurationAndODRHeader.h` is force-included rather than `#include`d, and 47
files use `LE_IMPL_NAMESPACE_BEGIN` without declaring where it comes from. With
`LE_SW_SDK_BUILD` gone that macro is unconditionally `namespace X {`, so this is
a mechanical deletion across those 47 files plus the IWYU pass.

**And the warning baseline this unblocks**, in order:

1. Add `-Wall -Wextra -Wno-unused-parameter` **to our targets only**, via the
   same per-source mechanism `sw_force_include_odr_header` already uses to decide
   what "ours" means — CMake cannot express it any other way, and 7.5 already
   paid for learning that. Sampling six representative files with those flags
   produced **425 further warnings**.
2. Deal with the 365 `-Wunknown-pragmas` in the same pass. They are all MSVC
   `#pragma warning`; either wrap them in `LE_MSVC_SPECIFIC` or add
   `-Wno-unknown-pragmas` and record why.
3. `-Werror` in CI only, once (1) and (2) are at zero. Never on a developer's
   build, where a new compiler version turning up a new warning should not stop
   work.

Third-party warnings — 6 sites, from rtaudio, rtmidi and the VST3 SDK, all
arriving through clap-wrapper's CPM fetches — are not ours. Do not patch them.
If they become noisy under a baseline, the lever is `SYSTEM` on the include
directories, applied to those targets only.
...
### The side chain has never been fed a different signal

Fourteen shipping effects take a side chain and every one of them is pinned only
where side == main — the one case in which a side-chain effect cannot be
distinguished from a bug that ignores the side chain entirely. The recipe, and
it is small:

1. **`tests/goldens/engineHarness.cpp`** — `render()` passes
   `inputPointers.data()` twice (`:181`). Give it a second signal and a second
   pointer array, generated from a *different* `signal` than the main one. The
   comment at `:166` already claims the side-chain effects "are driven
   separately"; this makes an existing comment true.
2. **A golden case per side-chain effect**, main and side deliberately dissimilar
   — a sine against noise is enough, and the corpus already has both generators.
3. **`tests/clap/pluginTests.cpp`** — `ActivePlugin::process` hardcodes
...
4. **Assert the Side Chain port's info.** The port test checks `count(true) == 2`
   and then only calls `ports->get(…, 0, …)`. One more call with index 1,
   asserting the id, the stereo type and that `IS_MAIN` is clear.
5. **The harness can feed a *file* now.** 25 golden fixtures hash identically on
...
- **Hand-written weak `strnlen`/`wcsnlen`** — `gui.cpp:1242-1270`, "OSX 10.6 does
  not provide std::strnlen", with the `!__LP64__` guard **commented out**. They
  are compiled into every macOS build today, in 2026.

### Formatting

**80 files fail `clang-format --dry-run -Werror`** (21.1.5), counted 03.08.2026,
and only **2** of them are outside `src/le/`. So it is one reformat commit and it
