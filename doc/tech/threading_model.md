...
| `LFOImpl::Timer`'s tempo and meter | **per timer, and a timer is per engine.** Relaxed `std::atomic` members, written by the audio thread once a block and read by the message thread to lay out the LFO panel; `Timer::timing()` takes both in one read. They were process-wide statics — two tracks at two tempi saw one tempo — and what made them shareable was that everything snapping a period was `static`; a `Timing` is an argument now. See issue #11 | ✅ |
...
| `PresetLoadReport` | a file-scope report the loader counts into and the caller takes; main thread only, and a restore shows none of it but a sample that did not load | ✅ |
